...
<script src="<%=basePath %>muti_select/src/MultiSelectDropList.js"></script>

<script type='text/javascript' src='/IQA/dwr/util.js'></script>
<script type='text/javascript' src='/IQA/dwr/interface/TableService.js'></script>
<script type='text/javascript' src='/IQA/dwr/engine.js'></script>



<link href="<%=basePath %>muti_select/css/multi.css" rel="stylesheet" type="text/css" />
<link href="<%=basePath%>css/style.css" rel="stylesheet" type="text/css" />
<link href="<%=basePath%>css/select.css" rel="stylesheet" type="text/css" />
<script type="text/javascript">
function getTableName(tableId){
	TableService.getTableNameById(tableId,function callback(name){
		var tbName = name;
		return tbName;
	})
}

function getAllAtributesForPie(tableId){
	//var tableName = getTableName(tableId)
	var tableName = "student";
	TableService.getAttibutes(tableName,function callback(list){
		//for(o in list){
		for(var i = 0;i<list.length;i++){
			document.getElementById("n_subclass").options.add(new Option(list[i],i));
		}
	})
}
...
function class_change(){
	...
	var picType = document.getElementById("n_class");
...
;$(function (){
	  $('.multi_select').MSDL({
	    'width': '160',
	    'data': ['sdadsa','dasdas','fasfasf','fsafas','qqqaa','dsadas','dddddd','dasdasdasd','dsadasdasd','dsadwdasd','dsadasdasd']
	  });
	});
...
<body>
    ...
    <ul>     
   	 <li>
       	...
       	<div style='display:none;' id="pie">
       		<label>变量 </label>
       		<select  name="n_subclass" id = "n_subclass" onchange="getsubtype()" >
       		<option selected="selected">请选择变量</option> 
       		</select>
       		<input type="hidden" id="maintype" name="maintype"/>
       		<input type="hidden" id="subtype" name="subtype"/>
       	</div>
       	<div style='display:none;' id="nonPie1">
       		<label>X轴变量 </label>
       		<select  name="n_subclass" id = "n_subclass" onchange="getsubtype()" >
       		<option selected="selected">请选择X轴变量</option> 
       		</select>
       		<input type="hidden" id="maintype" name="maintype"/>
       		<input type="hidden" id="subtype" name="subtype"/>
       	</div>
       	<div style='display:none;' id="nonPie2">
	       	<label>Y轴变量 </label>
