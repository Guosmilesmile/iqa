...
<script>
	...
	var education = [{ "value": "1", "text": "博士研究生" }, 
                  { "value": "2", "text": "大学本科" }, 
                  { "value": "3", "text": "硕士研究生" }, 
                  { "value": "4", "text": "专科生及以下" }];
					
	var degree = [{ "value": "0", "text": "无学位" }, 
                  { "value": "1", "text": "学士" }, 
                  { "value": "2", "text": "硕士" }, 
                  { "value": "3", "text": "博士" }];
					
	var professionaltitle = [{ "value": "1", "text": "教授" }, 
                  { "value": "2", "text": "副教授" }, 
                  { "value": "3", "text": "讲师" }, 
                  { "value": "4", "text": "助教" }, 
                  { "value": "5", "text": "其他正高级" }, 
	...
	function getData(queryParams){
		$('#grid').datagrid({
			...
			columns: [[
				...
				{field:'sli_id',title:'ID',sortable:true,width:80},
				{field:'sli_sequencenumber',title:'序号',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: {
						 type: 'numberbox',
					options: {
				...
				{field:'sli_name',title:'姓名',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox'}
				},
				{field:'sli_worknumber',title:'工号',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox'}
				},
				{field:'sli_position',title:'职务',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox'}
				},
				{field:'sli_gender',title:'性别',sortable:true,width:100,
					editor: 
					{ 
						 type : 'combobox',  
                         options : {  
				...
				{field:'sli_birthday',title:'出生年月',sortable:true,width:120,formatter:myformatter,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'datebox'}
				},
				{field:'sli_inschooldate',title:'入校时间',sortable:true,width:120,formatter:myformatter,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'datebox'}
				},
				{field:'sli_education',title:'学历',sortable:true,width:100,
					editor: 
					{ 
						 type : 'combobox',  
                         options : {  
				...
				{field:'sli_degree',title:'最高学位',sortable:true,width:100,
					editor: 
					{ 
						 type : 'combobox',  
                         options : {  
				...
				{field:'sli_professionaltitle',title:'专业技术职称',sortable:true,width:100,
					editor: 
					{ 
						 type : 'combobox',  
                         options : {  
				...
				{field:'sli_responsibility',title:'校内分管工作',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox'}
				},
				{field:'sli_studyworkresume',title:'学习和工作简历',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox'}
				},
				{field:'sli_comments',title:'审核意见',sortable:true,width:120
				}
			]],
			...
			onDblClickRow:function(rowIndex, rowData){
				...
				if(doedit==undefined)
				{					
					serialnumber = rowData.sli_serialnumber;
	...
	function _insertRow(){
		...
		if(doedit == undefined){
			...
			if(row!=null){
				...
				rowIndex = rowIndex + 1;
				serialnumber = row.sli_serialnumber + 1;
			}
			else{
				rowIndex = 0;
				serialnumber = 1;
	...
	function _editRow(){
		...
		if(row){
			...
			if(doedit == undefined){
				var rowIndex = $('#grid').datagrid('getRowIndex', row);
				serialnumber = row.sli_serialnumber;
				$('#grid').datagrid('beginEdit',rowIndex);
				doedit = rowIndex;
		...
		if(rows.length <= 0){
			...
			$.messager.confirm("操作警告", "确定删除后将不可恢复！！", function(data){
				if(data){
					...
					for(var i = 0; i < rows.length; ++i)
						{
							sm_id[i] = rows[i].sli_id;
						}						
					$.post("<%=basePath%>Sec_DeleteSchoolLeaderInfo", {sliids: sm_id.toString()},
	...
	function _saveRows(){
		$.messager.confirm("操作警告", "确定保存后被修改的数据将不可恢复！！", function(data){
			if(data){
				...
				for(var i = 0; i < rows.length; i++)
				{
					rows[i].sli_birthday = myparser(rows[i].sli_birthday);
					rows[i].sli_inschooldate = myparser(rows[i].sli_inschooldate);
				}
		        ...
		        if (inserted.length > 0) {  
		            url = '<%=basePath%>Sec_AddSchoolLeaderInfo';  
		        }  
		        if (updated.length > 0) {  
		        	url = '<%=basePath%>Sec_UpdateSchoolLeaderInfo';   
		        }  
		        ...
		        $.ajax({
		        	url: url,
		        	data: {rowdata:rowstr, serialnumber:serialnumber,sli_college:college},
		        	dataType: 'json',
		        	success : function(r){
		        		if(r==1){
		        			if(updated.length>0){
	...
	function _getnullcount(){
		$.messager.confirm("操作警告", "提交后数据将不可修改！！", function(data){
			if(data){
				$.post("<%=basePath%>Sec_GetSchoolLeaderInfo", {isnull: 1,sort:'sli_serialnumber',order:'asc',sli_college:college},
						function (data, textStatus){
						 data = $.parseJSON(data); 
						if(data.total==0){
							 _commitdata();
	...
	function _importdata(){
				...
				$('#ssearchDialog').show();
				if(!collegeflag){
					$("#ssearchDialog").attr("action","Sec_ImportSchoolLeaderInfo?tableid="+tableid+"&college="+encodeURI(encodeURI(college)));
		        }
				else{
					$("#ssearchDialog").attr("action","Sec_ImportSchoolLeaderInfo?tableid="+tableid+"&college="+encodeURI(college));
				}
