...
<title>附表6-1-6-1  各专业（大类）招生情况（时点）</title>
...
<script>
    ...
    $(function(){
		...
		if(college!=""&&college!=undefined)
		{
			...
			queryParams = {"mei_college":college};	
		}
		else{
			queryParams = {};
		}
	...
	function getData(queryParams){
		$('#grid').datagrid({
			url: '<%=basePath%>Sec_GetMajorEnrollInfoServlet',
			sortName: 'mei_serialnumber',
			sortOrder: 'asc',
			...
			columns: [[
				{field:'mei_majorcode',title:'校内专业（大类）代码',sortable:true,width:140,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox',}
				},
				{field:'mei_majorname',title:'校内专业（大类）名称',sortable:true,width:140,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox',}
				},
				{field:'mei_plannumber',title:'招生计划数（人）',sortable:true,width:100,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'mei_enrollnumber',title:'实际录取数（人）',sortable:true,width:100,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
			    ...
			    {field:'mei_comments',title:'审核意见',sortable:true,width:120}
			...
			onDblClickRow:function(rowIndex, rowData){
				...
				if(doedit==undefined)
				{					
					serialnumber = rowData.mei_serialnumber;
	...
	function _insertRow(){
		...
		if(doedit == undefined){
			...
			if(row!=null){
				...
				rowIndex = rowIndex + 1;
				serialnumber = row.mei_serialnumber + 1;
			}
			else{
				rowIndex = 0;
				serialnumber = 1;
	...
	function _editRow(){
		...
		if(row){
			...
			if(doedit == undefined){
				var rowIndex = $('#grid').datagrid('getRowIndex', row);
				serialnumber = row.mei_serialnumber;
				$('#grid').datagrid('beginEdit',rowIndex);
				doedit = rowIndex;
		...
		if(rows.length <= 0){
			...
			$.messager.confirm("操作警告", "确定删除后将不可恢复！！", function(data){
				if(data){
					...
					for(var i = 0; i < rows.length; ++i)
						{
							mei_id[i] = rows[i].mei_id;
						}						
					$.post("<%=basePath%>Sec_DeleteMajorEnrollInfoServlet", {meiids: mei_id.toString()},
		...
		$.messager.confirm("操作警告", "确定保存后被修改的数据将不可恢复！！", function(data){
			if(data){
	            ...
	            if (inserted.length > 0) {
	                url = '<%=basePath%>Sec_AddMajorEnrollInfoServlet';
	            }
	            if (updated.length > 0) {
	                url = '<%=basePath%>Sec_UpdateMajorEnrollInfoServlet';
	            }
		        ...
		        $.ajax({
		        	url: url,
		        	data: {rowdata:rowstr, serialnumber:serialnumber,mei_college:college},
		        	dataType: 'json',
		        	success : function(r){
		        		if(r==1){
		        			if(updated.length>0){
	...
	function _getnullcount(){
		$.messager.confirm("操作警告", "提交后数据将不可修改！！", function(data){
			if(data){
				$.post("<%=basePath%>Sec_GetMajorEnrollInfoServlet", {isnull: 1,mei_college:college,sort:'mei_serialnumber',order:'asc'},
						function (data, textStatus){
					 data = $.parseJSON(data); 
					if(data.total==0){
						 _commitdata();
	...
	function _importdata(){
		...
		$('#ssearchDialog').show();
		if(!collegeflag){
			$("#ssearchDialog").attr("action","Sec_ImportMajorEnrollInfoServlet?tableid="+tableid+"&college="+encodeURI(encodeURI(college)));
        }
		else{
			$("#ssearchDialog").attr("action","Sec_ImportMajorEnrollInfoServlet?tableid="+tableid+"&college="+encodeURI(college));
		}
...
<h2>附表6-1-6-1  各专业（大类）招生情况（时点）</h2>
	<table id="grid"></table>
