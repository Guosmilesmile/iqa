...
<script>
    ...
    var tableid,userid;
    var hlt_type = [{ "value": "1", "text": "国家杰出青年科学基金资助者" }, 
                             { "value": "2", "text": "教育部高校青年教师获奖者" }, 
                             { "value": "3", "text": "青年“千人计划”入选者" }, 
                             { "value": "4", "text": "省部级突出贡献专家" }, 
                             { "value": "5", "text": "省级高层次人才" }, 
                             { "value": "6", "text": "引进海外高层次人才“千人计划”入选者" }, 
                             { "value": "7", "text": "长江学者特聘教授" }, 
                             { "value": "8", "text": "中国工程院院士" }, 
                             { "value": "9", "text": "中国科学院院士 " }, 
                             { "value": "10", "text": "新世纪优秀人才 "}];
    $(function(){
    	$("#ssearchDialog").hide();
		//获取数据的查询参数----过滤数据
		//------------------------------------要加获取tableid的方法=======================
		...
		if(college!=""&&college!=undefined)
		{
			...
			queryParams = {"hlt_college":college};	
		}
		else{
			queryParams = {};
		}
	...
	function getData(queryParams){
		$('#grid').datagrid({
			...
			url: '<%=basePath%>Sec_GetHighLevelTalent',
			sortName: 'hlt_serialnumber',
			sortOrder: 'asc',
			...
			columns: [[
				...
				{field:'hlt_id',title:'ID',sortable:true,width:80},
				{field:'hlt_name',title:'姓名',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox'}
				},
				{field:'hlt_worknumber',title:'工号',sortable:true,width:120,
					editor: { type: 'validatebox'  }
				},
				{field:'hlt_type',title:'类型',sortable:true,width:235,
					editor: 
					{ 
						 type : 'combobox',  
                         options : {  
				...
				{field:'hlt_researchdirection',title:'研究方向',sortable:true,width:120,
					editor: { type: 'validatebox'  }
				},
				{field:'hlt_acquisitiondate',title:'获得时间',sortable:true,width:120,formatter:myformatter,
					editor: { type: 'datebox'  }
				},
				{field:'hlt_comments',title:'审核意见',sortable:true,width:120
				}
			]],
			...
			onDblClickRow:function(rowIndex, rowData){
				...
				if(doedit==undefined)
				{					
					serialnumber = rowData.hlt_serialnumber;
	...
	function _insertRow(){
		...
		if(doedit == undefined){
			...
			if(row!=null){
				...
				rowIndex = rowIndex + 1;
				serialnumber = row.hlt_serialnumber + 1;
			}
			else{
				rowIndex = 0;
				serialnumber = 1;
	...
	function _editRow(){
		...
		if(row){
			...
			if(doedit == undefined){
				var rowIndex = $('#grid').datagrid('getRowIndex', row);
				serialnumber = row.hlt_serialnumber;
				$('#grid').datagrid('beginEdit',rowIndex);
				doedit = rowIndex;
		...
		if(rows.length <= 0){
			...
			$.messager.confirm("操作警告", "确定删除后将不可恢复！！", function(data){
				if(data){
					...
					for(var i = 0; i < rows.length; ++i)
						{
							hlt_id[i] = rows[i].hlt_id;
						}						
					$.post("<%=basePath%>Sec_DeleteHighLevelTalent", {hltids: hlt_id.toString()},
	...
	function _saveRows(){
		$.messager.confirm("操作警告", "确定保存后被修改的数据将不可恢复！！", function(data){
			if(data){
				...
				for(var i = 0; i < rows.length; i++)
				{
					rows[i].hlt_acquisitiondate = myparser(rows[i].hlt_acquisitiondate);
				}
		        ...
		        if (inserted.length > 0) {  
		            url = '<%=basePath%>Sec_AddHighLevelTalent';  
		        }  
		        if (updated.length > 0) {  
		        	url = '<%=basePath%>Sec_UpdateHighLevelTalent';   
		        }  
		        ...
		        $.ajax({
		        	url: url,
		        	data: {rowdata:rowstr, serialnumber:serialnumber,hlt_college:college},
		        	dataType: 'json',
		        	success : function(r){
		        		if(r==1){
		        			if(updated.length>0){
	...
	function _getnullcount(){
		$.messager.confirm("操作警告", "提交后数据将不可修改！！", function(data){
			if(data){
				$.post("<%=basePath%>Sec_GetHighLevelTalent", {isnull: 1,sort:'hlt_serialnumber',order:'asc',hlt_college:college},
						function (data, textStatus){
						 data = $.parseJSON(data); 
						if(data.total==0){
							 _commitdata();
	...
	function _importdata(){
				...
				$('#ssearchDialog').show();
				if(!collegeflag){
					$("#ssearchDialog").attr("action","Sec_ImportHighLevelTalent?tableid="+tableid+"&college="+encodeURI(encodeURI(college)));
		        }
				else{
					$("#ssearchDialog").attr("action","Sec_ImportHighLevelTalent?tableid="+tableid+"&college="+encodeURI(college));
				}
