...
<title>附表4-2-3-1优势专业情况（时点）</title>
...
<script>
    ...
    $(function(){
		...
		if(college!=""&&college!=undefined)
		{
			...
			queryParams = {"ami_college":college};	
		}
		else{
			queryParams = {};
		}
	...
	function getData(queryParams){
		$('#grid').datagrid({
			url: '<%=basePath%>Sec_GetAdvantageMajorInfoServlet',
			sortName: 'ami_serialnumber',
			sortOrder: 'asc',
			...
			columns: [[
				{field:'ami_institute',title:'学院',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox',}
				},
				{field:'ami_name',title:'名称',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox',}
				},
				{field:'ami_type',title:'类别',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox',}
				},
				{field:'ami_nationallevel',title:'国家级',sortable:true,width:100,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'ami_provinciallevel',title:'省级',sortable:true,width:100,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'ami_remark',title:'备注',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox',}
				},
			    {field:'ami_comments',title:'审核意见',sortable:true,width:120}
			...
			onDblClickRow:function(rowIndex, rowData){
				...
				if(doedit==undefined)
				{					
					serialnumber = rowData.ami_serialnumber;
	...
	function _insertRow(){
		...
		if(doedit == undefined){
			...
			if(row!=null){
				...
				rowIndex = rowIndex + 1;
				serialnumber = row.ami_serialnumber + 1;
			}
			else{
				rowIndex = 0;
				serialnumber = 1;
	...
	function _editRow(){
		...
		if(row){
			...
			if(doedit == undefined){
				var rowIndex = $('#grid').datagrid('getRowIndex', row);
				serialnumber = row.ami_serialnumber;
				$('#grid').datagrid('beginEdit',rowIndex);
				doedit = rowIndex;
		...
		if(rows.length <= 0){
			...
			$.messager.confirm("操作警告", "确定删除后将不可恢复！！", function(data){
				if(data){
					...
					for(var i = 0; i < rows.length; ++i)
						{
							ami_id[i] = rows[i].ami_id;
						}						
					$.post("<%=basePath%>Sec_DeleteAdvantageMajorInfoServlet", {amiids: ami_id.toString() ,ami_college: college},
		...
		$.messager.confirm("操作警告", "确定保存后被修改的数据将不可恢复！！", function(data){
			if(data){
	            ...
	            if (inserted.length > 0) {
	                url = '<%=basePath%>Sec_AddAdvantageMajorInfoServlet';
	            }
	            if (updated.length > 0) {
	                url = '<%=basePath%>Sec_UpdateAdvantageMajorInfoServlet';
	            }
		        ...
		        $.ajax({
		        	url: url,
		        	data: {rowdata:rowstr, serialnumber:serialnumber,ami_college:college},
		        	dataType: 'json',
		        	success : function(r){
		        		if(r==1){
		        			if(updated.length>0){
	...
	function _getnullcount(){
		$.messager.confirm("操作警告", "提交后数据将不可修改！！", function(data){
			if(data){
				$.post("<%=basePath%>Sec_GetAdvantageMajorInfoServlet", {isnull: 1,ami_college:college,sort:'ami_serialnumber',order:'asc'},
						function (data, textStatus){
					 data = $.parseJSON(data); 
					if(data.total==0){
						 _commitdata();
	...
	function _importdata(){
		...
		$('#ssearchDialog').show();
		if(!collegeflag){
			$("#ssearchDialog").attr("action","Sec_ImportAdvantageMajorInfoServlet?tableid="+tableid+"&college="+encodeURI(encodeURI(college)));
        }
		else{
			$("#ssearchDialog").attr("action","Sec_ImportAdvantageMajorInfoServlet?tableid="+tableid+"&college="+encodeURI(college));
		}
