...
<title>表6-1-3近一届本科生招生类别情况（时点）</title>
...
<script>
    ...
    $(function(){
		...
		if(college!=""&&college!=undefined)
		{
			...
			queryParams = {"eci_college":college};	
		}
		else{
			queryParams = {};
		}
	...
	function getData(queryParams){
		$('#grid').datagrid({
			...
			columns: [[
			           {field:'eci_plannumber',title:'1.计划招生数',sortable:true,width:120,
					    	editor: {
								 type: 'numberbox',
							options: {
								min: 0,
					    ...
					    {field:'eci_enrollnumber',title:'2.实际录取数',sortable:true,width:120,
							//如果要实现编辑功能，需要添加下面的属性
							editor: {
								 type: 'numberbox',
							options: {
						...
						{field:'eci_registernumber',title:'3.实际报到数',sortable:true,width:120,
							//如果要实现编辑功能，需要添加下面的属性
							editor: {
								 type: 'numberbox',
							options: {
						...
						{field:'eci_indrecruitmentnumber',title:'4.自主招生数',sortable:true,width:120,
						//如果要实现编辑功能，需要添加下面的属性
						editor: {
							 type: 'numberbox',
						options: {
					...
					{field:'eci_specialnumber',title:'5.招收特长生数',sortable:true,width:100,
						editor: {
							 type: 'numberbox',
						options: {
							min: 0,
					...
					{field:'eci_provincenumber',title:'6.招收本省学生数',sortable:true,width:100,
						editor: {
							 type: 'numberbox',
						options: {
							min: 0,
					...
					{field:'eci_newspecialitynumber',title:'7.新办专业招生数',sortable:true,width:120,
						//如果要实现编辑功能，需要添加下面的属性
						editor: {
							 type: 'numberbox',
						options: {
	...
	function _insertRow(){
		...
		if(doedit == undefined){
			...
			if(row!=null){
				...
				rowIndex = rowIndex + 1;
				serialnumber = row.eci_serialnumber + 1;
			}
			else{
				rowIndex = 0;
				serialnumber = 1;
	...
	function _editRow(){
		...
		if(row){
			...
			if(doedit == undefined){
				var rowIndex = $('#grid').datagrid('getRowIndex', row);
				serialnumber = row.eci_serialnumber;
				$('#grid').datagrid('beginEdit',rowIndex);
				doedit = rowIndex;
		...
		if(rows.length <= 0){
			...
			$.messager.confirm("操作警告", "确定删除后将不可恢复！！", function(data){
				if(data){
					...
					for(var i = 0; i < rows.length; ++i)
						{
							eci_id[i] = rows[i].eci_id;
						}						
					$.post("<%=basePath%>Sec_DeleteEnrollCategoryInfoservlet", {eciids: eci_id.toString()},
		...
		$.messager.confirm("操作警告", "确定保存后被修改的数据将不可恢复！！", function(data){
			if(data){
	            ...
	            if (inserted.length > 0) {
	                url = '<%=basePath%>Sec_AddEnrollCategoryInfoservlet';
	            }
	            if (updated.length > 0) {
		        ...
		        $.ajax({
		        	url: url,
		        	data: {rowdata:rowstr, serialnumber:serialnumber,eci_college:college},
		        	dataType: 'json',
		        	success : function(r){
		        		if(r==1){
		        			if(updated.length>0){
	...
	function _getnullcount(){
		$.messager.confirm("操作警告", "提交后数据将不可修改！！", function(data){
			if(data){
				$.post("<%=basePath%>Sec_GetEnrollCategoryInfoservlet", {isnull: 1,eci_college:college,sort:'eci_serialnumber',order:'asc'},
						function (data, textStatus){
					 data = $.parseJSON(data); 
					if(data.total==0){
						 _commitdata();
	...
	function _importdata(){
		...
		$('#ssearchDialog').show();
		if(!collegeflag){
			$("#ssearchDialog").attr("action","Sec_ImportEnrollCategoryInfoservlet?tableid="+tableid+"&college="+encodeURI(encodeURI(college)));
        }
		else{
			$("#ssearchDialog").attr("action","Sec_ImportEnrollCategoryInfoservlet?tableid="+tableid+"&college="+encodeURI(college));
		}
