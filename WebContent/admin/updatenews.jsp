...
<script type="text/javascript">
	...
	function getmenu(){
		...
		var param = {};
		MenuDao.getAllMenu(param,function callback(data){ 
			for(o in data){
	...
	function getnewsinfo(){
		...
		var titles = data.n_titles;
		var version = data.n_version;
		
		var all_options = document.getElementById("n_class").options;
		for(i=0;i<all_options.length;i++){
			if(all_options[i].text == mainclass)
				{
	...
	//	$("#n_class").find("option[text="+mainclass+"]").attr("selected",true);	
		
		$("#n_subclass").find("option[text="+subclass+"]").attr("selected",true);
		$("#n_version").find("option[text="+version+"]").attr("selected",true);
		$("#n_id").val(id);
		$("#n_titles").val(titles);
		$("#n_author").val(author);
		$("#d121").val(time);
		$("#n_summary").val(summary);
		document.getElementsByTagName("iframe")[0].contentWindow.document.body.innerHTML = content;
	});
...
<body>
    ...
    <div class="formtext">Hi，<b>admin</b>，欢迎您使用新闻修改功能</div>
   	 	<form action="<%=basePath%>updatenews" method="post" enctype="multipart/form-data">
        	<ul class="forminfo">     
            	 <li>
             	...
             	<li>
                	...
                	<input class="dfinput" id ="n_titles" name="n_titles" type="text"  placeholder="请填写新闻标题" style="width:518px;"/>
               </li>
               <li>
               		<label>新闻作者<b>*</b></label>
               		<input class="dfinput" id="n_author" name="n_author" type="text"  placeholder="请填写新闻作者"  style="width:518px;"/>
   				...
   				<li>
               		<label>新闻概要<b>*</b></label>
               		<input class="dfinput" id="n_summary" name="n_summary" type="text"  placeholder="请填写新闻概要"  style="width:518px;"/>
               </li>
               <li><label>通知内容<b>*</b></label>
                   <textarea id="editor" name="n_content" rows="" cols="" style="width: 800px; height: 200px; visibility: visible;">
		 			</textarea>
               </li> 
