...
<title>附表6-1-5-3本科生招生志愿满足率（时点）</title>
...
<script>
    ...
    $(function(){
		...
		if(college!=""&&college!=undefined)
		{
			...
			queryParams = {"uer_college":college};	
		}
		else{
			queryParams = {};
		}
	...
	function getData(queryParams){
		$('#grid').datagrid({
			url: '<%=basePath%>Sec_GetUndergraEnrollRateServlet',
			sortName: 'uer_serialnumber',
			sortOrder: 'asc',
			...
			columns: [[
				{field:'uer_institute',title:' 学院',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox',}
				},
				{field:'uer_admission',title:'录取专业',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox',}
				},
				{field:'uer_firstmajorrate',title:'第一专业志愿满足率',sortable:true,width:140,
					//如果要实现编辑功能，需要添加下面的属性
					editor: {
						 type: 'numberbox',
					options: {
				...
				{field:'uer_unfirstmajorrate',title:'非第一专业志愿满足率',sortable:true,width:150,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'uer_adjustrate',title:'调剂率',sortable:true,width:100,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
			    ...
			    {field:'uer_comments',title:'审核意见',sortable:true,width:120}
			...
			onDblClickRow:function(rowIndex, rowData){
				...
				if(doedit==undefined)
				{					
					serialnumber = rowData.uer_serialnumber;
	...
	function _insertRow(){
		...
		if(doedit == undefined){
			...
			if(row!=null){
				...
				rowIndex = rowIndex + 1;
				serialnumber = row.uer_serialnumber + 1;
			}
			else{
				rowIndex = 0;
				serialnumber = 1;
	...
	function _editRow(){
		...
		if(row){
			...
			if(doedit == undefined){
				var rowIndex = $('#grid').datagrid('getRowIndex', row);
				serialnumber = row.uer_serialnumber;
				$('#grid').datagrid('beginEdit',rowIndex);
				doedit = rowIndex;
		...
		if(rows.length <= 0){
			...
			$.messager.confirm("操作警告", "确定删除后将不可恢复！！", function(data){
				if(data){
					...
					for(var i = 0; i < rows.length; ++i)
						{
							uer_id[i] = rows[i].uer_id;
						}						
					$.post("<%=basePath%>Sec_DeleteUndergraEnrollRateServlet", {uerids: uer_id.toString()},
		...
		$.messager.confirm("操作警告", "确定保存后被修改的数据将不可恢复！！", function(data){
			if(data){
	            ...
	            if (inserted.length > 0) {
	                url = '<%=basePath%>Sec_AddUndergraEnrollRateServlet';
	            }
	            if (updated.length > 0) {
	                url = '<%=basePath%>Sec_UpdateUndergraEnrollRateServlet';
	            }
		        ...
		        $.ajax({
		        	url: url,
		        	data: {rowdata:rowstr, serialnumber:serialnumber,uer_college:college},
		        	dataType: 'json',
		        	success : function(r){
		        		if(r==1){
		        			if(updated.length>0){
	...
	function _getnullcount(){
		$.messager.confirm("操作警告", "提交后数据将不可修改！！", function(data){
			if(data){
				$.post("<%=basePath%>Sec_GetUndergraEnrollRateServlet", {isnull: 1,uer_college:college,sort:'uer_serialnumber',order:'asc'},
						function (data, textStatus){
					 data = $.parseJSON(data); 
					if(data.total==0){
						 _commitdata();
	...
	function _importdata(){
		...
		$('#ssearchDialog').show();
		if(!collegeflag){
			$("#ssearchDialog").attr("action","Sec_ImportUndergraEnrollRateServlet?tableid="+tableid+"&college="+encodeURI(encodeURI(college)));
        }
		else{
			$("#ssearchDialog").attr("action","Sec_ImportUndergraEnrollRateServlet?tableid="+tableid+"&college="+encodeURI(college));
		}
