...
<script>
    ...
    $(function(){
		...
		if(college!=""&&college!=undefined)
		{
			...
			queryParams = {"usic_college":college};	
		}
		else{
			queryParams = {};
		}
	...
	function getData(queryParams){
		$('#grid').datagrid({
			...
			url: '<%=basePath%>Sec_GetUndergraStuInlandCommu',
			sortName: 'usic_serialnumber',
			sortOrder: 'asc',
			...
			columns: [[
				...
				{field:'usic_id',title:'ID',sortable:true,width:80},
				{field:'usic_institute',title:'学院',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox'}
				},
				{field:'usic_outnumber',title:'本校到外校学生数',sortable:true,width:120,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'usic_innumber',title:'外校到本校学生数',sortable:true,width:120,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'usic_comments',title:'审核意见',sortable:true,width:120
				}
			]],
			...
			onDblClickRow:function(rowIndex, rowData){
				...
				if(doedit==undefined)
				{					
					serialnumber = rowData.usic_serialnumber;
	...
	function _insertRow(){
		...
		if(doedit == undefined){
			...
			if(row!=null){
				...
				rowIndex = rowIndex + 1;
				serialnumber = row.usic_serialnumber + 1;
			}
			else{
				rowIndex = 0;
				serialnumber = 1;
	...
	function _editRow(){
		...
		if(row){
			...
			if(doedit == undefined){
				var rowIndex = $('#grid').datagrid('getRowIndex', row);
				serialnumber = row.usic_serialnumber;
				$('#grid').datagrid('beginEdit',rowIndex);
				doedit = rowIndex;
		...
		if(rows.length <= 0){
			...
			$.messager.confirm("操作警告", "确定删除后将不可恢复！！", function(data){
				if(data){
					...
					for(var i = 0; i < rows.length; ++i)
						{
							usic_id[i] = rows[i].usic_id;
						}						
					$.post("<%=basePath%>Sec_DeleteUndergraStuInlandCommu", {usicids: usic_id.toString(),usic_college:college},
		...
		$.messager.confirm("操作警告", "确定保存后被修改的数据将不可恢复！！", function(data){
			if(data){
		        ...
		        if (inserted.length > 0) {  
		            url = '<%=basePath%>Sec_AddUndergraStuInlandCommu';  
		        }  
		        if (updated.length > 0) {  
		        	url = '<%=basePath%>Sec_UpdateUndergraStuInlandCommu';   
		        }  
		        ...
		        $.ajax({
		        	url: url,
		        	data: {rowdata:rowstr, serialnumber:serialnumber,usic_college:college},
		        	dataType: 'json',
		        	success : function(r){
		        		if(r==1){
		        			if(updated.length>0){
	...
	function _getnullcount(){
		$.messager.confirm("操作警告", "提交后数据将不可修改！！", function(data){
			if(data){
				$.post("<%=basePath%>Sec_GetUndergraStuInlandCommu", {isnull: 1,sort:'usic_serialnumber',order:'asc',usic_college:college},
						function (data, textStatus){
						 data = $.parseJSON(data); 
						if(data.total==0){
							 _commitdata();
	...
	function _importdata(){
				...
				$('#ssearchDialog').show();
				if(!collegeflag){
					$("#ssearchDialog").attr("action","Sec_ImportUndergraStuInlandCommu?tableid="+tableid+"&college="+encodeURI(encodeURI(college)));
		        }
				else{
					$("#ssearchDialog").attr("action","Sec_ImportUndergraStuInlandCommu?tableid="+tableid+"&college="+encodeURI(college));
				}
	...
	//下载模板
		...
		function onLoadSuccess(data) {
			...
			for (var i = 0; i < rows.length; i++) {
					if(rows[i].usic_institute == "合计")
						totalI=i;
					if(totalI==0)
						break;
					else{
						var temp = rows[0];
						var total = rows[totalI];
						$('#grid').datagrid('getData').rows[0] = total;
			            $('#grid').datagrid('getData').rows[totalI] = temp;
			            $('#grid').datagrid('refreshRow', 0);
			            $('#grid').datagrid('refreshRow', toalI);
					}
			}
		}
</script>
