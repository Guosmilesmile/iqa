...
<title>表6-1-6-2 各专业（大类）报到情况（时点）</title>
...
<script>
    ...
    $(function(){
		...
		if(college!=""&&college!=undefined)
		{
			...
			queryParams = {"mri_college":college};	
		}
		else{
			queryParams = {};
		}
	...
	function getData(queryParams){
		$('#grid').datagrid({
			url: '<%=basePath%>Sec_GetMajorRegisterInfoServlet',
			sortName: 'mri_serialnumber',
			sortOrder: 'asc',
			...
			columns: [[
				{field:'mri_majorcode',title:'校内专业（大类）代码',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox',}
				},
				{field:'mri_majorname',title:'校内专业（大类）名称',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox',}
				},
				{field:'mri_registernumber',title:'实际报到数（人）',sortable:true,width:100,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
			    ...
			    {field:'mri_comments',title:'审核意见',sortable:true,width:120}
			...
			onDblClickRow:function(rowIndex, rowData){
				...
				if(doedit==undefined)
				{					
					serialnumber = rowData.mri_serialnumber;
	...
	function _insertRow(){
		...
		if(doedit == undefined){
			...
			if(row!=null){
				...
				rowIndex = rowIndex + 1;
				serialnumber = row.mri_serialnumber + 1;
			}
			else{
				rowIndex = 0;
				serialnumber = 1;
	...
	function _editRow(){
		...
		if(row){
			...
			if(doedit == undefined){
				var rowIndex = $('#grid').datagrid('getRowIndex', row);
				serialnumber = row.mri_serialnumber;
				$('#grid').datagrid('beginEdit',rowIndex);
				doedit = rowIndex;
		...
		if(rows.length <= 0){
			...
			$.messager.confirm("操作警告", "确定删除后将不可恢复！！", function(data){
				if(data){
					...
					for(var i = 0; i < rows.length; ++i)
						{
							mri_id[i] = rows[i].mri_id;
						}						
					$.post("<%=basePath%>Sec_DeleteMajorRegisterInfoServlet", {mriids: mri_id.toString()},
		...
		$.messager.confirm("操作警告", "确定保存后被修改的数据将不可恢复！！", function(data){
			if(data){
	            ...
	            if (inserted.length > 0) {
	                url = '<%=basePath%>Sec_AddMajorRegisterInfoServlet';
	            }
	            if (updated.length > 0) {
	                url = '<%=basePath%>Sec_UpdateMajorRegisterInfoServlet';
	            }
		        ...
		        $.ajax({
		        	url: url,
		        	data: {rowdata:rowstr, serialnumber:serialnumber,mri_college:college},
		        	dataType: 'json',
		        	success : function(r){
		        		if(r==1){
		        			if(updated.length>0){
	...
	function _getnullcount(){
		$.messager.confirm("操作警告", "提交后数据将不可修改！！", function(data){
			if(data){
				$.post("<%=basePath%>Sec_GetMajorRegisterInfoServlet", {isnull: 1,mri_college:college,sort:'mri_serialnumber',order:'asc'},
						function (data, textStatus){
					 data = $.parseJSON(data); 
					if(data.total==0){
						 _commitdata();
	...
	function _importdata(){
		...
		$('#ssearchDialog').show();
		if(!collegeflag){
			$("#ssearchDialog").attr("action","Sec_ImportMajorRegisterInfoServlet?tableid="+tableid+"&college="+encodeURI(encodeURI(college)));
        }
		else{
			$("#ssearchDialog").attr("action","Sec_ImportMajorRegisterInfoServlet?tableid="+tableid+"&college="+encodeURI(college));
		}
