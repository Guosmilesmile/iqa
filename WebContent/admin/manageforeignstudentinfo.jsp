...
<title>表6-1-4  国外及港澳台学生情况（时点）</title>
...
<script>
    ...
    $(function(){
		...
		if(college!=""&&college!=undefined)
		{
			...
			queryParams = {"fsi_college":college};	
		}
		else{
			queryParams = {};
		}
		...
		$('#grid').datagrid({
			...
			columns: [[
			           {title:'本科生（境外）',colspan:4,rowspan:2},
						...
						{field:'fsi_allgraduatenumber',title:'毕（结）业生人数（人）',sortable:true,width:140,
							editor: {
								 type: 'numberbox',
							options: {
								min: 0,
						...
						{field:'fsi_foreigngraduatenumber',title:'毕（结）业生人数（人）',sortable:true,width:140,
							editor: {
								 type: 'numberbox',
							options: {
								min: 0,
						...
						{field:'fsi_hkgraduatenumber',title:'毕（结）业生人数（人）',sortable:true,width:140,
							editor: {
								 type: 'numberbox',
							options: {
								min: 0,
						...
						{field:'fsi_macgraduatenumber',title:'毕（结）业生人数（人）',sortable:true,width:140,
							editor: {
								 type: 'numberbox',
							options: {
								min: 0,
						...
						{field:'fsi_twngraduatenumber',title:'毕（结）业生人数（人）',sortable:true,width:140,
							editor: {
								 type: 'numberbox',
							options: {
								min: 0,
	...
	function _insertRow(){
		...
		if(doedit == undefined){
			...
			if(row!=null){
				...
				rowIndex = rowIndex + 1;
				serialnumber = row.fsi_serialnumber + 1;
			}
			else{
				rowIndex = 0;
				serialnumber = 1;
	...
	function _editRow(){
		...
		if(row){
			...
			if(doedit == undefined){
				var rowIndex = $('#grid').datagrid('getRowIndex', row);
				serialnumber = row.fsi_serialnumber;
				$('#grid').datagrid('beginEdit',rowIndex);
				doedit = rowIndex;
		...
		if(rows.length <= 0){
			...
			$.messager.confirm("操作警告", "确定删除后将不可恢复！！", function(data){
				if(data){
					...
					for(var i = 0; i < rows.length; ++i)
						{
							fsi_id[i] = rows[i].fsi_id;
						}						
					$.post("<%=basePath%>Sec_DeleteForeignStudentInfoServlet", {fsiids: fsi_id.toString()},
		...
		$.messager.confirm("操作警告", "确定保存后被修改的数据将不可恢复！！", function(data){
			if(data){
	            ...
	            if (inserted.length > 0) {
	                url = '<%=basePath%>Sec_AddForeignStudentInfoServlet';
	            }
	            if (updated.length > 0) {
		        ...
		        $.ajax({
		        	url: url,
		        	data: {rowdata:rowstr, serialnumber:serialnumber,fsi_college:college},
		        	dataType: 'json',
		        	success : function(r){
		        		if(r==1){
		        			if(updated.length>0){
	...
	function _getnullcount(){
		$.messager.confirm("操作警告", "提交后数据将不可修改！！", function(data){
			if(data){
				$.post("<%=basePath%>Sec_GetForeignStudentInfoServlet", {isnull: 1,fsi_college:college,sort:'fsi_serialnumber',order:'asc'},
						function (data, textStatus){
					 data = $.parseJSON(data); 
					if(data.total==0){
						 _commitdata();
	...
	function _importdata(){
		...
		$('#ssearchDialog').show();
		if(!collegeflag){
			$("#ssearchDialog").attr("action","Sec_ImportForeignStudentInfoServlet?tableid="+tableid+"&college="+encodeURI(encodeURI(college)));
        }
		else{
			$("#ssearchDialog").attr("action","Sec_ImportForeignStudentInfoServlet?tableid="+tableid+"&college="+encodeURI(college));
		}
