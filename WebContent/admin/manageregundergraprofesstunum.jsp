...
<script>
    ...
    $(function(){
		...
		if(college!=""&&college!=undefined)
		{
			...
			queryParams = {"rupsn_college":college};	
		}
		else{
			queryParams = {};
		}
	...
	function getData(queryParams){
		$('#grid').datagrid({
			...
			columns: [[
			    {field:'rupsn_id',title:'ID',sortable:true,width:80,rowspan:2},
			    ...
			    {field:'rupsn_comments',title:'审核意见',sortable:true,width:120,rowspan:2}
			    ],[
				
				{field:'rupsn_atschtotal',title:'总计',sortable:true,width:120},
				{field:'rupsn_gradeone',title:'一年级',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: {
						 type: 'numberbox',
					options: {
				...
				{field:'rupsn_gradefour',title:'四年级',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: {
						 type: 'numberbox',
					options: {
				...
				{field:'rupsn_abgradefive',title:'五年级及以上',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: {
						 type: 'numberbox',
					options: {
				...
				{field:'rupsn_minor',title:'其中：辅修',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: {
						 type: 'numberbox',
					options: {
				...
				{field:'rupsn_doubledegree',title:'其中：双学位',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: {
						 type: 'numberbox',
					options: {
				...
				{field:'rupsn_intonumber',title:'转入人数',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: {
						 type: 'numberbox',
					options: {
				...
				{field:'rupsn_outnumber',title:'转出人数',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: {
						 type: 'numberbox',
					options: {
			...
			onDblClickRow:function(rowIndex, rowData){
				...
				if(doedit==undefined)
				{					
					serialnumber = rowData.rupsn_serialnumber;
	...
	function _insertRow(){
		...
		if(doedit == undefined){
			...
			if(row!=null){
				...
				rowIndex = rowIndex + 1;
				serialnumber = row.rupsn_serialnumber + 1;
			}
			else{
				rowIndex = 0;
				serialnumber = 1;
	...
	function _editRow(){
		...
		if(row){
			...
			if(doedit == undefined){
				var rowIndex = $('#grid').datagrid('getRowIndex', row);
				serialnumber = row.rupsn_serialnumber;
				$('#grid').datagrid('beginEdit',rowIndex);
				doedit = rowIndex;
		...
		if(rows.length <= 0){
			...
			$.messager.confirm("操作警告", "确定删除后将不可恢复！！", function(data){
				if(data){
					...
					for(var i = 0; i < rows.length; ++i)
						{
							rupsn_id[i] = rows[i].rupsn_id;
						}						
					$.post("<%=basePath%>Sec_DeleteRegUndergraProfesStuNum", {rupsnids: rupsn_id.toString()},
		...
		$.messager.confirm("操作警告", "确定保存后被修改的数据将不可恢复！！", function(data){
			if(data){
	            ...
	            if (inserted.length > 0) {
	                url = '<%=basePath%>Sec_AddRegUndergraProfesStuNum';
	            }
	            if (updated.length > 0) {
		        ...
		        $.ajax({
		        	url: url,
		        	data: {rowdata:rowstr, serialnumber:serialnumber,rupsn_college:college},
		        	dataType: 'json',
		        	success : function(r){
		        		if(r==1){
		        			if(updated.length>0){
	...
	function _getnullcount(){
		$.messager.confirm("操作警告", "提交后数据将不可修改！！", function(data){
			if(data){
				$.post("<%=basePath%>Sec_GetRegUndergraProfesStuNum", {isnull: 1,sort:'rupsn_serialnumber',order:'asc',rupsn_college:college},
						function (data, textStatus){
						 data = $.parseJSON(data); 
						if(data.total==0){
							 _commitdata();
	...
	function _importdata(){
				...
				$('#ssearchDialog').show();
				if(!collegeflag){
					$("#ssearchDialog").attr("action","Sec_ImportRegUndergraProfesStuNum?tableid="+tableid+"&college="+encodeURI(encodeURI(college)));
		        }
				else{
					$("#ssearchDialog").attr("action","Sec_ImportRegUndergraProfesStuNum?tableid="+tableid+"&college="+encodeURI(college));
				}
