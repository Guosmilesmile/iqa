...
<title>附表6-2-1-8厦门大学学生体质健康情况（学年）</title>
...
<script>
    ...
    $(function(){
		...
		if(college!=""&&college!=undefined)
		{
			...
			queryParams = {"sphi_college":college};	
		}
		else{
			queryParams = {};
		}
	...
	function getData(queryParams){
		$('#grid').datagrid({
			url: '<%=basePath%>Sec_GetStuPhysicalHealthInfoServlet',
			sortName: 'sphi_serialnumber',
			sortOrder: 'asc',
			...
			columns: [[
				{field:'sphi_grade',title:'年级',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox',}
				},
				{field:'sphi_totalnumber',title:'总人数',sortable:true,width:100,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'sphi_freetestnumber',title:'免于测试人数',sortable:true,width:100,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'sphi_testnumber',title:'测试人数',sortable:true,width:100,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'sphi_passnumber',title:'合格人数',sortable:true,width:100,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'sphi_goodnumber',title:'良人数',sortable:true,width:100,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'sphi_excellentnumber',title:'优秀人数',sortable:true,width:100,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
			    ...
			    {field:'sphi_comments',title:'审核意见',sortable:true,width:120}
			...
			onDblClickRow:function(rowIndex, rowData){
				...
				if(doedit==undefined)
				{					
					serialnumber = rowData.sphi_serialnumber;
	...
	function _insertRow(){
		...
		if(doedit == undefined){
			...
			if(row!=null){
				...
				rowIndex = rowIndex + 1;
				serialnumber = row.sphi_serialnumber + 1;
			}
			else{
				rowIndex = 0;
				serialnumber = 1;
	...
	function _editRow(){
		...
		if(row){
			...
			if(doedit == undefined){
				var rowIndex = $('#grid').datagrid('getRowIndex', row);
				serialnumber = row.sphi_serialnumber;
				$('#grid').datagrid('beginEdit',rowIndex);
				doedit = rowIndex;
		...
		if(rows.length <= 0){
			...
			$.messager.confirm("操作警告", "确定删除后将不可恢复！！", function(data){
				if(data){
					...
					for(var i = 0; i < rows.length; ++i)
						{
							sphi_id[i] = rows[i].sphi_id;
						}						
					$.post("<%=basePath%>Sec_DeleteStuPhysicalHealthInfoServlet", {sphiids: sphi_id.toString() ,sphi_college:college},
		...
		$.messager.confirm("操作警告", "确定保存后被修改的数据将不可恢复！！", function(data){
			if(data){
	            ...
	            if (inserted.length > 0) {
	                url = '<%=basePath%>Sec_AddStuPhysicalHealthInfoServlet';
	            }
	            if (updated.length > 0) {
	                url = '<%=basePath%>Sec_UpdateStuPhysicalHealthInfoServlet';
	            }
		        ...
		        $.ajax({
		        	url: url,
		        	data: {rowdata:rowstr, serialnumber:serialnumber,sphi_college:college},
		        	dataType: 'json',
		        	success : function(r){
		        		if(r==1){
		        			if(updated.length>0){
	...
	function _getnullcount(){
		$.messager.confirm("操作警告", "提交后数据将不可修改！！", function(data){
			if(data){
				$.post("<%=basePath%>Sec_GetStuPhysicalHealthInfoServlet", {isnull: 1,sphi_college:college,sort:'sphi_serialnumber',order:'asc'},
						function (data, textStatus){
					 data = $.parseJSON(data); 
					if(data.total==0){
						 _commitdata();
	...
	function _importdata(){
		...
		$('#ssearchDialog').show();
		if(!collegeflag){
			$("#ssearchDialog").attr("action","Sec_ImportStuPhysicalHealthInfoServlet?tableid="+tableid+"&college="+encodeURI(encodeURI(college)));
        }
		else{
			$("#ssearchDialog").attr("action","Sec_ImportStuPhysicalHealthInfoServlet?tableid="+tableid+"&college="+encodeURI(college));
		}
...
<h2>附表6-2-1-8厦门大学学生体质健康情况（学年）</h2>
	<table id="grid"></table>
