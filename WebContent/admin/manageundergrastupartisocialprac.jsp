...
<script>
    ...
    $(function(){
		...
		if(college!=""&&college!=undefined)
		{
			...
			queryParams = {"uspsp_college":college};	
		}
		else{
			queryParams = {};
		}
	...
	function getData(queryParams){
		$('#grid').datagrid({
			...
			url: '<%=basePath%>Sec_GetUndergraStuPartiSocialPrac',
			sortName: 'uspsp_serialnumber',
			sortOrder: 'desc',
			...
			columns: [[
				...
				{field:'uspsp_id',title:'ID',sortable:true,width:80},
				{field:'uspsp_department',title:'单位',sortable:true,width:120,
					editor : {  
                        type : 'combobox',  
                        options : {  
                            ...
                            onChange : function (newValue, oldValue) {  
                               ...
                               var ed2 = $('#grid').datagrid('getEditor', { index: rindex, field: 'uspsp_department' });
                               $(ed2.target).combobox('setValue', tt[0]);
                            } 
                        }  
                    }  
				},
				{field:'uspsp_focuspracnum',title:'参加集中社会实践人数',sortable:true,width:120,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'uspsp_scatterpracnum',title:'分散社会实践人数',sortable:true,width:120,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'uspsp_subtotal',title:'小计',sortable:true,width:120},
				{field:'uspsp_comments',title:'审核意见',sortable:true,width:120
				}
			]],
	...
	function _insertRow(){
		...
		if(doedit == undefined){
			...
			if(row!=null){
				...
				rowIndex = rowIndex + 1;
				serialnumber = row.uspsp_serialnumber + 1;
			}
			else{
				rowIndex = 0;
				serialnumber = 1;
	...
	function _editRow(){
		...
		if(row){
			...
			if(doedit == undefined){
				var rowIndex = $('#grid').datagrid('getRowIndex', row);
				serialnumber = row.uspsp_serialnumber;
				$('#grid').datagrid('beginEdit',rowIndex);
				doedit = rowIndex;
		...
		if(rows.length <= 0){
			...
			$.messager.confirm("操作警告", "确定删除后将不可恢复！！", function(data){
				if(data){
					...
					for(var i = 0; i < rows.length; ++i)
						{
							uspsp_id[i] = rows[i].uspsp_id;
						}						
					$.post("<%=basePath%>Sec_DeleteUndergraStuPartiSocialPrac", {uspspids: uspsp_id.toString(),uspsp_college:college},
		...
		$.messager.confirm("操作警告", "确定保存后被修改的数据将不可恢复！！", function(data){
			if(data){
		        ...
		        if (inserted.length > 0) {  
		            url = '<%=basePath%>Sec_AddUndergraStuPartiSocialPrac';  
		        }  
		        if (updated.length > 0) {  
		        	url = '<%=basePath%>Sec_UpdateUndergraStuPartiSocialPrac';   
		        }  
		        ...
		        $.ajax({
		        	url: url,
		        	data: {rowdata:rowstr, serialnumber:serialnumber,uspsp_college:college},
		        	dataType: 'json',
		        	success : function(r){
		        		if(r==1){
		        			if(updated.length>0){
	...
	function _getnullcount(){
		$.messager.confirm("操作警告", "提交后数据将不可修改！！", function(data){
			if(data){
				$.post("<%=basePath%>Sec_GetUndergraStuPartiSocialPrac", {isnull: 1,sort:'uspsp_serialnumber',order:'asc',uspsp_college:college},
						function (data, textStatus){
						 data = $.parseJSON(data); 
						if(data.total==0){
							 _commitdata();
	...
	function _importdata(){
				...
				$('#ssearchDialog').show();
				if(!collegeflag){
					$("#ssearchDialog").attr("action","Sec_ImportUndergraStuPartiSocialPrac?tableid="+tableid+"&college="+encodeURI(encodeURI(college)));
		        }
				else{
					$("#ssearchDialog").attr("action","Sec_ImportUndergraStuPartiSocialPrac?tableid="+tableid+"&college="+encodeURI(college));
				}
