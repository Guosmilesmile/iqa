...
<title>附表3-5-1-1教师教学发展机构（学年）</title>
...
<script>
    ...
    $(function(){
		...
		if(college!=""&&college!=undefined)
		{
			...
			queryParams = {"dati_college":college};	
		}
		else{
			queryParams = {};
		}
	...
	function getData(queryParams){
		$('#grid').datagrid({
			url: '<%=basePath%>Sec_GetDevAgencyTrainInfoServlet',
			sortName: 'dati_serialnumber',
			sortOrder: 'asc',
			...
			columns: [[
				{field:'dati_name',title:'机构名称',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox',}
				},
				{field:'dati_departmentname',title:'单位名称',sortable:true,width:140,
	            	 editor: { type: 'combobox',
	                 	options : {  
	                         ...
	                         onChange : function (newValue, oldValue) {  
	                            ...
	                            var ed1 = $('#grid').datagrid('getEditor', { index: rindex, field: 'dati_departmentnumber' });
	                            var ed2 = $('#grid').datagrid('getEditor', { index: rindex, field: 'dati_departmentname' });
	                            if(tt.length > 1)
	                         	   $(ed1.target).val(tt[1]);
				...
				{field:'dati_departmentnumber',title:'单位号',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox',}
				},
				{field:'dati_workplan',title:'工作计划',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: { type: 'validatebox',}
				},
				{field:'dati_traintimes',title:'培训次数',sortable:true,width:100,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'dati_traintrips',title:'培训人次',sortable:true,width:100,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
			    ...
			    {field:'dati_comments',title:'审核意见',sortable:true,width:120}
	...
	function _insertRow(){
		...
		if(doedit == undefined){
			...
			if(row!=null){
				...
				rowIndex = rowIndex + 1;
				serialnumber = row.dati_serialnumber + 1;
			}
			else{
				rowIndex = 0;
				serialnumber = 1;
	...
	function _editRow(){
		...
		if(row){
			...
			if(doedit == undefined){
				var rowIndex = $('#grid').datagrid('getRowIndex', row);
				serialnumber = row.dati_serialnumber;
				$('#grid').datagrid('beginEdit',rowIndex);
				doedit = rowIndex;
		...
		if(rows.length <= 0){
			...
			$.messager.confirm("操作警告", "确定删除后将不可恢复！！", function(data){
				if(data){
					...
					for(var i = 0; i < rows.length; ++i)
						{
							dati_id[i] = rows[i].dati_id;
						}						
					$.post("<%=basePath%>Sec_DeleteDevAgencyTrainInfoServlet", {datiids: dati_id.toString()},
		...
		$.messager.confirm("操作警告", "确定保存后被修改的数据将不可恢复！！", function(data){
			if(data){
	            ...
	            if (inserted.length > 0) {
	                url = '<%=basePath%>Sec_AddDevAgencyTrainInfoServlet';
	            }
	            if (updated.length > 0) {
	                url = '<%=basePath%>Sec_UpdateDevAgencyTrainInfoServlet';
	            }
		        ...
		        $.ajax({
		        	url: url,
		        	data: {rowdata:rowstr, serialnumber:serialnumber,dati_college:college},
		        	dataType: 'json',
		        	success : function(r){
		        		if(r==1){
		        			if(updated.length>0){
	...
	function _getnullcount(){
		$.messager.confirm("操作警告", "提交后数据将不可修改！！", function(data){
			if(data){
				$.post("<%=basePath%>Sec_GetDevAgencyTrainInfoServlet", {isnull: 1,dati_college:college,sort:'dati_serialnumber',order:'asc'},
						function (data, textStatus){
					 data = $.parseJSON(data); 
					if(data.total==0){
						 _commitdata();
	...
	function _importdata(){
		...
		$('#ssearchDialog').show();
		if(!collegeflag){
			$("#ssearchDialog").attr("action","Sec_ImportDevAgencyTrainInfoServlet?tableid="+tableid+"&college="+encodeURI(encodeURI(college)));
        }
		else{
			$("#ssearchDialog").attr("action","Sec_ImportDevAgencyTrainInfoServlet?tableid="+tableid+"&college="+encodeURI(college));
		}
