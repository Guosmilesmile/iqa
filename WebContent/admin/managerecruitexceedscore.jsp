...
<base href="<%=basePath%>">
<title>厦门大学在全国各省（市、自治区）招生出档分数情况表</title>
...
<script>
    ...
    function getYear()
    {
    	var year = [{"value":"0", "text":"1970"}];
    	var newYear = (new Date()).getFullYear();
    	year.pop();
    	for(var i = 0; i <= 30; i++)
    	{
    		year.push({"value":i+"","text":(newYear-i)+""});
    	}
    	return year;
    }
    $(function(){
		...
		if(college!=""&&college!=undefined)
		{
			...
			queryParams = {"res_college":college};	
		}
		else{
			queryParams = {};
		}
	...
	function getData(queryParams){
		$('#grid').datagrid({
			...
			url: '<%=basePath%>Sec_GetRecruitExceedScore',
			sortName: 'res_serialnumber',
			sortOrder: 'asc',
			...
			columns: [[
				{field:'res_id',title:'ID',sortable:true,width:80,rowspan:2},
// 				{field:'res_year',title:'年份',sortable:true,width:150,rowspan:2,editor: {
// 					 type: 'numberbox',
// 						options: {
// 							min: 0,
// 							max: 9999,
...
// 							}
// 						}},
				{field:'res_year',title:'年份',sortable:true,width:50,rowspan:2,editor: {
					type : 'combobox',  
					options : {  
			    ...
			    {title:'文科',colspan:3},
			    {title:'理科',colspan:3},
			    {field:'res_comments',title:'审核意见',sortable:true,width:120,rowspan:2}
			    ],[
				{field:'res_libexctwentyproportion',title:'超出20分的省份比例(%)',sortable:true,width:135,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'res_libexcthirtyproportion',title:'超出30分的省份比例(%)',sortable:true,width:135,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'res_libexclineave',title:'所有生源省份出档线超出本一线的平均值',sortable:true,width:225,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'res_sciexcthirtyproportion',title:'超出30分的省份比例(%)',sortable:true,width:135,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'res_sciexcfortyproportion',title:'超出40分的省份比例(%)',sortable:true,width:135,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'res_sciexclineave',title:'所有生源省份出档线超出本一线的平均值',sortable:true,width:225,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
			...
			onDblClickRow:function(rowIndex, rowData){
				...
				if(doedit==undefined)
				{					
					serialnumber = rowData.res_serialnumber;
	...
	function _insertRow(){
		...
		if(doedit == undefined){
			...
			if(row!=null){
				...
				rowIndex = rowIndex + 1;
				serialnumber = row.res_serialnumber + 1;
			}
			else{
				rowIndex = 0;
				serialnumber = 1;
	...
	function _editRow(){
		...
		if(row){
			...
			if(doedit == undefined){
				var rowIndex = $('#grid').datagrid('getRowIndex', row);
				serialnumber = row.res_serialnumber;
				$('#grid').datagrid('beginEdit',rowIndex);
				doedit = rowIndex;
		...
		if(rows.length <= 0){
			...
			$.messager.confirm("操作警告", "确定删除后将不可恢复！！", function(data){
				if(data){
					...
					for(var i = 0; i < rows.length; ++i)
						{
							res_id[i] = rows[i].res_id;
						}						
					$.post("<%=basePath%>Sec_DeleteRecruitExceedScore", {resids: res_id.toString(),res_college:college},
		...
		$.messager.confirm("操作警告", "确定保存后被修改的数据将不可恢复！！", function(data){
			if(data){
		        ...
		        if (inserted.length > 0) {  
		            url = '<%=basePath%>Sec_AddRecruitExceedScore';  
		        }  
		        if (updated.length > 0) {  
		        	url = '<%=basePath%>Sec_UpdateRecruitExceedScore';   
		        }  
		        ...
		        $.ajax({
		        	url: url,
		        	data: {rowdata:rowstr, serialnumber:serialnumber,res_college:college},
		        	dataType: 'json',
		        	success : function(r){
		        		if(r==1){
		        			if(updated.length>0){
	...
	function _getnullcount(){
		$.messager.confirm("操作警告", "提交后数据将不可修改！！", function(data){
			if(data){
				$.post("<%=basePath%>Sec_GetRecruitExceedScore", {isnull: 1,sort:'res_serialnumber',order:'asc',res_college:college},
						function (data, textStatus){
						 data = $.parseJSON(data); 
						if(data.total==0){
							 _commitdata();
	...
	function _importdata(){
				...
				$('#ssearchDialog').show();
				if(!collegeflag){
					$("#ssearchDialog").attr("action","Sec_ImportRecruitExceedScore?tableid="+tableid+"&college="+encodeURI(encodeURI(college)));
		        }
				else{
					$("#ssearchDialog").attr("action","Sec_ImportRecruitExceedScore?tableid="+tableid+"&college="+encodeURI(college));
				}
...
<h2>附表6-1-5-1厦门大学在全国各省（市、自治区）招生出档分数情况</h2>
	<table id="grid"></table>
