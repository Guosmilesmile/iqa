...
<script>
    ...
    $(function(){
		...
		if(college!=""&&college!=undefined)
		{
			...
			queryParams = {"as_college":college};	
		}
		else{
			queryParams = {};
		}
	...
	function getData(queryParams){
		$('#grid').datagrid({
			url: '<%=basePath%>Sec_GetAlumnusAndSocialCoopServlet',
			sortName: 'as_serialnumber',
			sortOrder: 'asc',
			...
			columns: [[
			    ...
			    {title:'1.校友会（个）',colspan:3},
			    {title:'2.签订合作协议机构（个）',colspan:4},
			    {field:'as_comments',title:'审核意见',sortable:true,width:120,rowspan:2}
			    ],[
				{field:'as_alumnusamount',title:'校友会总数',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: {
						 type: 'numberbox',
					options: {
				...
				{field:'as_domesticalumnus',title:'其中：境内',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: {
						 type: 'numberbox',
					options: {
				...
				{field:'as_overseaalumnus',title:'境外',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: {
						 type: 'numberbox',
					options: {
				...
				{field:'as_agencyamount',title:'机构总数',sortable:true,width:100,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'as_academic',title:'其中：学术机构',sortable:true,width:100,
					editor: {
						 type: 'numberbox',
					options: {
						min: 0,
				...
				{field:'as_industry',title:'行业机构和企业',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: {
						 type: 'numberbox',
					options: {
				...
				{field:'as_government',title:'地方政府',sortable:true,width:120,
					//如果要实现编辑功能，需要添加下面的属性
					editor: {
						 type: 'numberbox',
					options: {
			...
			onDblClickRow:function(rowIndex, rowData){
				...
				if(doedit==undefined)
				{					
					serialnumber = rowData.as_serialnumber;
	...
	function _insertRow(){
		...
		if(doedit == undefined){
			...
			if(row!=null){
				...
				rowIndex = rowIndex + 1;
				serialnumber = row.as_serialnumber + 1;
			}
			else{
				rowIndex = 0;
				serialnumber = 1;
	...
	function _editRow(){
		...
		if(row){
			...
			if(doedit == undefined){
				var rowIndex = $('#grid').datagrid('getRowIndex', row);
				serialnumber = row.as_serialnumber;
				$('#grid').datagrid('beginEdit',rowIndex);
				doedit = rowIndex;
		...
		if(rows.length <= 0){
			...
			$.messager.confirm("操作警告", "确定删除后将不可恢复！！", function(data){
				if(data){
					...
					for(var i = 0; i < rows.length; ++i)
						{
							as_id[i] = rows[i].as_id;
						}						
					$.post("<%=basePath%>Sec_DeleteAlumnusAndSocialCoopServlet", {asids: as_id.toString()},
		...
		$.messager.confirm("操作警告", "确定保存后被修改的数据将不可恢复！！", function(data){
			if(data){
	            ...
	            if (inserted.length > 0) {
	                url = '<%=basePath%>Sec_AddAlumnusAndSocialCoopServlet';
	            }
	            if (updated.length > 0) {
	                url = '<%=basePath%>Sec_UpdateAlumnusAndSocialCoopServlet';
	            }
		        ...
		        $.ajax({
		        	url: url,
		        	data: {rowdata:rowstr, serialnumber:serialnumber,as_college:college},
		        	dataType: 'json',
		        	success : function(r){
		        		if(r==1){
		        			if(updated.length>0){
	...
	function _getnullcount(){
		$.messager.confirm("操作警告", "提交后数据将不可修改！！", function(data){
			if(data){
				$.post("<%=basePath%>Sec_GetAlumnusAndSocialCoopServlet", {isnull: 1,as_college:college,sort:'as_serialnumber',order:'asc'},
						function (data, textStatus){
					 data = $.parseJSON(data); 
					if(data.total==0){
						 _commitdata();
	...
	function _importdata(){
		...
		$('#ssearchDialog').show();
		if(!collegeflag){
			$("#ssearchDialog").attr("action","Sec_ImportAlumnusAndSocialCoopServlet?tableid="+tableid+"&college="+encodeURI(encodeURI(college)));
        }
		else{
			$("#ssearchDialog").attr("action","Sec_ImportAlumnusAndSocialCoopServlet?tableid="+tableid+"&college="+encodeURI(college));
		}
	...
	//下载模板
		function _downloadModel(){
			...
			location.hrsef="<%=basePath%>downloadModelServlet?tableid="+tableid;
