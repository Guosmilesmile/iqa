...
function toPage(currentPage,key){
	if(key.indexOf("'") >= 0 || title.indexOf("--") >= 0)
	{
		alert("请勿输入无效字符，如‘、--等");
		return;
	}
	window.location.href="GetNewsWithCondition?page="+currentPage+"&key="+key;
}
...
function getQueryNews(key){
	...
	$.ajax({
        type: 'post',
        url: "<%=basePath%>GetNewsWithCondition",
        data: {
        	pagination:"true",
        	rows:pageSize,
        	page:currentPage,
        ...
        success: function(newsListJson){
        	...
        	if(newsList.length > 0){
        		...
        		var emptyStr = "<tr><td><div class='question'><h3>未搜索相关信息</h3>"
        		    +"</div></td></tr>";
...
$(document).ready(function () { 
	...
	var key = decodeURI(escape(request['key']));
	getQueryNews(key);
});
    </script>
    <style>
        ...
        <div id="main" class="container container-fluid" >
            <br class="row-fluid">
                <div id="sub-nav" class="span3" >
                    <span class="sub-nav-title">搜索结果</span>
                    <!-- left menu ================================================== -->
                    <ul>
