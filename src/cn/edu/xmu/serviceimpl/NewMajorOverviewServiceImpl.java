 ...
 *
 */
public class NewMajorOverviewServiceImpl implements NewMajorOverviewService{
	...
	public List<NewMajorOverview> getNewMajorOverview() {
		...
		List<NewMajorOverview> nmoList = new ArrayList<NewMajorOverview>();
		int serialNumber = 0;
		
		List<MajorInfo> miList = miDao.getNewMajor();
		...
		{		
			...
			Float firstWork = null; //初次就业率
				...
				nmoList.add(new NewMajorOverview(++serialNumber, majorName, majorSetYear,
						selfTeacher, otherTeacher, highTeacher, undergraduates, stuVsTeacher, stuFlow, graduates, firstWork));							
		}
		
		//降序排序
		Collections.sort(nmoList);
		for (int i = 0; i < nmoList.size(); i++)
			nmoList.get(i).setSerialNumber(i + 1);// 重置序号
