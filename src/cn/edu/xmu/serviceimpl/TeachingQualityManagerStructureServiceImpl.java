...
public class TeachingQualityManagerStructureServiceImpl implements TeachingQualityManagerStructureService{
	...
	public List<TeachingQualityManagerStructure> getteManagerStructuresOfSchoolQM(Map params){
		...
		String rowNames[] = {"校级教学质量管理队伍数量","比例（%）"};//行名称
		...
		params.put(ManagerInfoTable.MI_MANAGERTYPE, "教学管理人员");
		...
		TeachingQualityManagerStructure row1 = new TeachingQualityManagerStructure(rowNames[0], Integer.toString(total), Integer.toString(zhenggaoji), Integer.toString(fugaoji), Integer.toString(zhongji), Integer.toString(chuji), Integer.toString(noTitle), Integer.toString(doctor), Integer.toString(master), Integer.toString(bachelor), Integer.toString(noDegree), Integer.toString(below35), Integer.toString(between36_45), Integer.toString(between46_55), Integer.toString(upon56), college);
		TeachingQualityManagerStructure row2 = new TeachingQualityManagerStructure(rowNames[1], "/", Double.toString(zhenggaojiPer), Double.toString(fugaojiPer), Double.toString(zhongjiPer), Double.toString(chujiPer), Double.toString(noTitlePer), Double.toString(doctorPer), Double.toString(masterPer), Double.toString(bachelorPer), Double.toString(noDegreePer), Double.toString(below35Per), Double.toString(between36_45Per), Double.toString(between46_55Per), Double.toString(upon56Per), college);
		
		List<TeachingQualityManagerStructure> teachingQualityManagerStructures  = new ArrayList<TeachingQualityManagerStructure>();
		teachingQualityManagerStructures.add(row1);
		teachingQualityManagerStructures.add(row2);
		return teachingQualityManagerStructures;
	}

	@Override
	public List<TeachingQualityManagerStructure> getteManagerStructuresOfCollegeQM(
			Map params) {
		String rowNames[] = {"院系教学质量管理队伍数量","比例（%）"};//行名称
		...
		between36_45 = managerInfoDao.getCountByRangeInTeachingScientific(ManagerInfoTable.MI_BIRTHDAY, start, end,params);
		percent = total > 0 ?(double)between36_45 / total * 100 : 0;
		between36_45Per = new BigDecimal(percent).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();  

		start.setYear(now.getYear() - 55);
		end.setYear(now.getYear() - 46);
		between46_55 = managerInfoDao.getCountByRangeInTeachingScientific(ManagerInfoTable.MI_BIRTHDAY, start, end,params);
		percent = total > 0 ?(double)between46_55 / total * 100 : 0;
		between46_55Per = new BigDecimal(percent).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();  
	...
	@Override
	public List<TeachingQualityManagerStructure> getteManagerStructuresOfQMor(
			Map params) {
		String rowNames[] = {"质量监控人员队伍","比例（%）"};//行名称
		...
		params.put(ManagerInfoTable.MI_MANAGERTYPE, "教学质量监控人员");
		total = managerInfoDao.getManagerInfoCount(params);
		
		params.put(ManagerInfoTable.MI_PROFESSIONALTITLE, "正高级");
		zhenggaoji = managerInfoDao.getManagerInfoCount(params);
		percent = total > 0 ?(double)zhenggaoji / total * 100 : 0;
		zhenggaojiPer = new BigDecimal(percent).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();  

		params.put(ManagerInfoTable.MI_PROFESSIONALTITLE, "副高级");
		fugaoji = managerInfoDao.getManagerInfoCount(params);
		percent = total > 0 ?(double)fugaoji / total * 100 : 0;
		fugaojiPer = new BigDecimal(percent).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();  

		params.put(ManagerInfoTable.MI_PROFESSIONALTITLE, "中级");
		zhongji = managerInfoDao.getManagerInfoCount(params);
		percent = total > 0 ?(double)zhongji / total * 100 : 0;
		zhongjiPer = new BigDecimal(percent).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();  

		params.put(ManagerInfoTable.MI_PROFESSIONALTITLE, "初级");
		chuji = managerInfoDao.getManagerInfoCount(params);
		percent = total > 0 ?(double)chuji / total * 100 : 0;
		chujiPer = new BigDecimal(percent).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();  

		params.put(ManagerInfoTable.MI_PROFESSIONALTITLE, "未评级");
		noTitle = managerInfoDao.getManagerInfoCount(params);
		percent = total > 0 ?(double)noTitle / total * 100 : 0;
		noTitlePer = new BigDecimal(percent).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();  
		...
		params.put(ManagerInfoTable.MI_DEGREES, "博士");
		doctor = managerInfoDao.getManagerInfoCount(params);
		percent = total > 0 ?(double)doctor / total * 100 : 0;
		doctorPer = new BigDecimal(percent).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();  

		params.put(ManagerInfoTable.MI_DEGREES, "硕士");
		master = managerInfoDao.getManagerInfoCount(params);
		percent = total > 0 ?(double)master / total * 100 : 0;
		masterPer = new BigDecimal(percent).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();  

		params.put(ManagerInfoTable.MI_DEGREES, "学士");
		bachelor = managerInfoDao.getManagerInfoCount(params);
		percent = total > 0 ?(double)bachelor / total * 100 : 0;
		bachelorPer = new BigDecimal(percent).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();  

		params.put(ManagerInfoTable.MI_DEGREES, "无学位");
		noDegree = managerInfoDao.getManagerInfoCount(params);
		percent = total > 0 ?(double)noDegree / total * 100 : 0;
		noDegreePer = new BigDecimal(percent).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();  
		...
		below35 = managerInfoDao.getCountByRange(ManagerInfoTable.MI_BIRTHDAY, start, end,params);
		percent = total > 0 ?(double)below35 / total * 100 : 0;
		below35Per = new BigDecimal(percent).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();  

		start.setYear(now.getYear() - 45);
		end.setYear(now.getYear() - 36);
		between36_45 = managerInfoDao.getCountByRange(ManagerInfoTable.MI_BIRTHDAY, start, end,params);
		percent = total > 0 ?(double)between36_45 / total * 100 : 0;
		between36_45Per = new BigDecimal(percent).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();  

		start.setYear(now.getYear() - 55);
		end.setYear(now.getYear() - 46);
		between46_55 = managerInfoDao.getCountByRange(ManagerInfoTable.MI_BIRTHDAY, start, end,params);
		percent = total > 0 ?(double)between46_55 / total * 100 : 0;
		between46_55Per = new BigDecimal(percent).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();  

		start.setYear(now.getYear() - 100);
		end.setYear(now.getYear() - 56);
		upon56 = managerInfoDao.getCountByRange(ManagerInfoTable.MI_BIRTHDAY, start, end,params);
		percent = total > 0 ?(double)upon56 / total * 100 : 0;
		upon56Per = new BigDecimal(percent).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();  
	...
	@Override
	public List<TeachingQualityManagerStructure> getteManagerStructure(
			Map params) {
		
		List<TeachingQualityManagerStructure> schoolQMs  = getteManagerStructuresOfSchoolQM(params);
		List<TeachingQualityManagerStructure> collegeQMs  = getteManagerStructuresOfCollegeQM(params);
		List<TeachingQualityManagerStructure> qmor  = getteManagerStructuresOfQMor(params);

		for (TeachingQualityManagerStructure teachingQualityManagerStructure : collegeQMs) {
			schoolQMs.add(teachingQualityManagerStructure);
		}
		
		for (TeachingQualityManagerStructure teachingQualityManagerStructure : qmor) {
			schoolQMs.add(teachingQualityManagerStructure);
		}
