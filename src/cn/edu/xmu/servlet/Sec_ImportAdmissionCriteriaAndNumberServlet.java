...
@WebServlet("/Sec_ImportAdmissionCriteriaAndNumberServlet")
public class Sec_ImportAdmissionCriteriaAndNumberServlet extends HttpServlet{
	 ...
	 */
	public Sec_ImportAdmissionCriteriaAndNumberServlet() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request,
		...
		int recordcount = 0;
		List<AdmissionCriteriaAndNumber> tsList = new ArrayList<AdmissionCriteriaAndNumber>();
		...
		try {
			...
			if (tablename.equals("附表6-1-5-4近一届本科生录取标准及人数（时点）")) {
			    tsList = getAlltsByExcel(completeFilePath);
			    acnDao.deleteByCollegeandDeadline(college, null);
				recordcount = tsList.size();
				for (int i = 0; i < tsList.size(); i++) {
					acnDao.addAdmissionCriteriaAndNumberRecord(tsList.get(i));
				}
			}
	 ...
	 * @return
	 */
	public static List<AdmissionCriteriaAndNumber> getAlltsByExcel(String file) {
		errorrow = 1;
		List<AdmissionCriteriaAndNumber> acnList = new ArrayList<AdmissionCriteriaAndNumber>();
		try {
			Workbook rwb = Workbook.getWorkbook(new File(file));
			Sheet rs = rwb.getSheet(0);// 或者rwb.getSheet(0)
			//int clos = rs.getColumns();// 得到所有的列
			int rows = getRightRows(rs);// 得到所有的行
			System.out.println("clos:" +9 + " rows:" + rows);
			for (int i = 3; i < rows; i++) {
				for (int j = 0; j < 9; j++) {
					// 第一个是列数，第二个是行数
					String acn_province = rs.getCell(j++, i).getContents();// 默认最左边编号也算一列,所以这里得j++
					String acn_batch = rs.getCell(j++, i).getContents();
					
					String artsadmission = rs.getCell(j++, i).getContents();
					Integer acn_artsadmission = -999;
					if(!artsadmission.equals(""))
						acn_artsadmission = Integer.valueOf(artsadmission);
					String scienceadmission = rs.getCell(j++, i).getContents();
					Integer acn_scienceadmission = -999;
					if(!scienceadmission.equals(""))
						acn_scienceadmission = Integer.valueOf(scienceadmission);
					String artsminctrline = rs.getCell(j++, i).getContents();
					Double acn_artsminctrline = Double.valueOf("-999");
					if(!artsminctrline.equals(""))
						acn_artsminctrline = Double.valueOf(artsminctrline);
					String scienceminctrline = rs.getCell(j++, i).getContents();
					...
					String artsavgscore = rs.getCell(j++, i).getContents();
					Double acn_artsavgscore = Double.valueOf("-999");
					if(!artsavgscore.equals(""))
						acn_artsavgscore = Double.valueOf(artsavgscore);
					String scienceavgscore =  rs.getCell(j++, i).getContents();
					Double acn_scienceavgscore = Double.valueOf("-999");
					if(!scienceavgscore.equals(""))
						acn_scienceavgscore = Double.valueOf(scienceavgscore);
//					int acn_artsadmission =Integer.parseInt( rs.getCell(j++, i).getContents());
//					int acn_scienceadmission =Integer.parseInt( rs.getCell(j++, i).getContents());
//					double acn_artsminctrline = Double.parseDouble( rs.getCell(j++, i).getContents());
//					double acn_scienceminctrline = Double.parseDouble( rs.getCell(j++, i).getContents());
//					double acn_artsavgscore = Double.parseDouble( rs.getCell(j++, i).getContents());
//					double acn_scienceavgscore = Double.parseDouble( rs.getCell(j++, i).getContents());
					String acn_instruction = rs.getCell(j++, i).getContents();
					...
					acnList.add(new AdmissionCriteriaAndNumber(acn_province, acn_batch, acn_artsadmission,
							acn_scienceadmission, acn_artsminctrline, acn_scienceminctrline, acn_artsavgscore,
							acn_scienceavgscore, acn_instruction, college,isnull));
				}
				errorrow++;
			}
		} catch (Exception e) {
