...
@WebServlet("/Sec_ImportUndergraEnrollRateServlet")
public class Sec_ImportUndergraEnrollRateServlet extends HttpServlet{
	 ...
	 */
	public Sec_ImportUndergraEnrollRateServlet() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request,
		...
		int recordcount = 0;
		List<UndergraEnrollRate> tsList = new ArrayList<UndergraEnrollRate>();
		...
		try {
			...
			if (tablename.equals("附表6-1-5-3本科生招生志愿满足率（时点）")) {
			    tsList = getAlltsByExcel(completeFilePath);
			    uerDao.deleteByCollegeandDeadline(college, null);
				recordcount = tsList.size();
				for (int i = 0; i < tsList.size(); i++) {
					uerDao.addUndergraEnrollRateRecord(tsList.get(i));
				}
			}
	 ...
	 * @return
	 */
	public static List<UndergraEnrollRate> getAlltsByExcel(String file) {
		errorrow = 1;
		List<UndergraEnrollRate> ueriList = new ArrayList<UndergraEnrollRate>();
		try {
			...
			for (int i = 2; i < rows; i++) {
				for (int j = 0; j < 5; j++) {
					// 第一个是列数，第二个是行数
					String uer_institute = rs.getCell(j++, i).getContents();// 默认最左边编号也算一列,所以这里得j++
					String uer_admission = rs.getCell(j++, i).getContents();
					
					String firstmajorrate = ( rs.getCell(j++, i).getContents());
					Double uer_firstmajorrate = Double.valueOf("-999");
					if(!firstmajorrate.equals(""))
						uer_firstmajorrate = Double.valueOf(firstmajorrate);
					String unfirstmajorrate = ( rs.getCell(j++, i).getContents());
					...
					String adjustrate = ( rs.getCell(j++, i).getContents());
					double uer_adjustrate = Double.valueOf("-999");
					if(!adjustrate.equals(""))
						uer_adjustrate =  Double.valueOf(adjustrate);
//					double uer_firstmajorrate = Double.parseDouble( rs.getCell(j++, i).getContents());
//					double uer_unfirstmajorrate = Double.parseDouble( rs.getCell(j++, i).getContents());
//					double uer_adjustrate = Double.parseDouble( rs.getCell(j++, i).getContents());
					...
					ueriList.add(new UndergraEnrollRate(uer_institute, uer_admission, uer_firstmajorrate,
							uer_unfirstmajorrate, uer_adjustrate, college,isnull));
				}
				errorrow++;
			}
		} catch (Exception e) {
