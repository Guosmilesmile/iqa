 ...
 *
 */
@WebServlet("/Sec_ImportGraduateEmployAsMajorServlet")
public class Sec_ImportGraduateEmployAsMajorServlet extends HttpServlet
{
	...
	public Sec_ImportGraduateEmployAsMajorServlet()
	{
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		int recordcount = 0;
		List<GraduateEmployAsMajor> feList = new ArrayList<GraduateEmployAsMajor>();

		TableListDao tableListDao = new TableListDaoImpl();
		GraduateEmployAsMajorDao feDao = new GraduateEmployAsMajorDaoImpl();
		...
		try
		{
			...
			if (tablename.equals("附表6-1-9-3应届本科毕业生分专业就业情况（时点）"))
			{
				...
				{
					feDao.addGraduateEmployAsMajor(feList.get(i));
				}
			}
	 ...
	 * @return
	 */
	public static List<GraduateEmployAsMajor> getAlltsByExcel(String file)
	{
		errorrow = 1;
		List<GraduateEmployAsMajor> feList = new ArrayList<GraduateEmployAsMajor>();
		try
		{
			...
			for (int i = 2; i < rows; i++)
			{
				...
				{
					// 第一个是列数，第二个是行数
					String geam_majorcodeinschool = rs.getCell(j++, i).getContents();
					String geam_majornameinschool = rs.getCell(j++, i).getContents();			
					String graduatenum = rs.getCell(j++, i).getContents();
					...
					GraduateEmployAsMajor graduateEmployAsMajor = new GraduateEmployAsMajor(
							geam_majorcodeinschool, geam_majornameinschool, geam_graduatenum,college, 1, geam_isnull);
					
					feList.add(graduateEmployAsMajor);
