...
@WebServlet("/Sec_ImportCourseBuildInfoServlet")
public class Sec_ImportCourseBuildInfoServlet extends HttpServlet{
	 ...
	 */
	public Sec_ImportCourseBuildInfoServlet() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request,
		...
		int recordcount = 0;
		List<CourseBuildInfo> tsList = new ArrayList<CourseBuildInfo>();

		TableListDao tableListDao = new TableListDaoImpl();
		CourseBuildInfoDao miDao = new CourseBuildInfoDaoImpl();
	 ...
	 * @return
	 */
	public static List<CourseBuildInfo> getAlltsByExcel(String file) {
		errorrow = 1;
		List<CourseBuildInfo> cbiList = new ArrayList<CourseBuildInfo>();
		try {
			...
			for (int i = 2; i < rows; i++) {
				for (int j = 0; j < 6; j++) {
					// 第一个是列数，第二个是行数
					String cbi_institute = rs.getCell(j++, i).getContents();// 默认最左边编号也算一列,所以这里得j++
					String cbi_name = rs.getCell(j++, i).getContents();
					String cbi_chargeperson = rs.getCell(j++, i).getContents();
					String cbi_type = rs.getCell(j++, i).getContents();
					String cbi_grade = rs.getCell(j++, i).getContents();
					String approvaltime = rs.getCell(j++, i).getContents();
					...
					cbiList.add(new CourseBuildInfo(cbi_institute, cbi_name, cbi_chargeperson, cbi_type, 
							cbi_grade, cbi_approvaltime, college,isnull));
				}
				errorrow++;
			}
		} catch (Exception e) {
