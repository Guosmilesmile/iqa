...
public class UploadAccessoryOver extends HttpServlet {
	...
	// 定义允许上传的文件的扩展名
	protected String[] FILETYPES = new String[] { "doc", "docx", "wps", "xls",
			"ppt", "pptx", "pdf", "txt", "rar", "zip" };
	// 定义上传文件保存目录路径
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 文件保存目录路径
		String uploadPath = new StringBuffer(request.getSession()
				.getServletContext().getRealPath("/")).append(savePath)
				.toString();

		System.out.println("==="+uploadPath);
		// 文件保存目录URL
//		String saveUrl = new StringBuffer(request.getContextPath()).append("/")
//				.append(savePath).toString();
		...
		try {
			...
			while (itr.hasNext()) {
				...
				if (((String) item.getFieldName()).equals("attachTitle")) {
					...
					if (attachTitle != null) {
						attachTitle = new String(
								attachTitle.getBytes("ISO8859-1"), "UTF-8");
					}
				}
				if (!item.isFormField()) {
					// 检查文件大小
					...
					// 根据时间创建文件夹
					SimpleDateFormat folderNameFormat = new SimpleDateFormat(
							"yyyyMMdd");
					String realPath = uploadPath
							+ folderNameFormat.format(new Date());
					...
					if (flag) {
						SimpleDateFormat fileNameFormat = new SimpleDateFormat(
								"yyyyMMddHHmmss");
						String newFileName = fileNameFormat.format(new Date())
								+ "_" + new Random().nextInt(1000) + "."
								+ fileExt;
						File uploadedFile = new File(realPath, newFileName);
						item.write(uploadedFile);
						saveUrl += folderNameFormat.format(new Date()) + "/"
								+ newFileName;
						ext = fileExt;
					} else {
	...
	public String alertMsg(String message) {
		...
		sb.append("alert(\"").append(message)
				.append("\");history.back();</script>");
	...
	public String insertAttach(String id, String url, String title, String ext) {
		...
		sb.append("<body>");
		sb.append("<script type=\"text/javascript\">");
		sb.append("parent.KE.plugin[\"accessory\"].insert(\"").append(id)
				.append("\",\"");
		sb.append(url).append("\",\"").append(title).append("\",\"")
				.append(ext).append("\");</script>");
		sb.append("</body>").append("</html>");
		return sb.toString();
	}
