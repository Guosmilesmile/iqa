...
@WebServlet("/Sec_ImportPostdoctoralMobileStationServlet")
public class Sec_ImportPostdoctoralMobileStationServlet extends HttpServlet{
	 ...
	 */
	public Sec_ImportPostdoctoralMobileStationServlet() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request,
		...
		int recordcount = 0;
		List<PostdoctoralMobileStation> tsList = new ArrayList<PostdoctoralMobileStation>();
		...
		try {
			...
			if (tablename.equals("表4-1-2博士后流动站（时点）")) {
			    tsList = getAlltsByExcel(completeFilePath);
			    pmsDao.deleteByCollegeandDeadline(college, null);
				recordcount = tsList.size();
				for (int i = 0; i < tsList.size(); i++) {
					pmsDao.addPostdoctoralMobileStationRecord(tsList.get(i));
				}
			}
	 ...
	 * @return
	 */
	public static List<PostdoctoralMobileStation> getAlltsByExcel(String file) {
		errorrow = 1;
		List<PostdoctoralMobileStation> pmsList = new ArrayList<PostdoctoralMobileStation>();
		try {
			Workbook rwb = Workbook.getWorkbook(new File(file));
			Sheet rs = rwb.getSheet(0);// 或者rwb.getSheet(0)
			//int clos = rs.getColumns();// 得到所有的列
			int rows = getRightRows(rs);// 得到所有的行
			System.out.println("clos:" +3 + " rows:" + rows);
			for (int i = 2; i < rows; i++) {
				for (int j = 0; j < 3; j++) {
					// 第一个是列数，第二个是行数
					String pms_departmentnumber = rs.getCell(j++, i).getContents();// 默认最左边编号也算一列,所以这里得j++
					String pms_departmentname = rs.getCell(j++, i).getContents();
					String pms_stationname = rs.getCell(j++, i).getContents();
					...
					pmsList.add(new PostdoctoralMobileStation(pms_departmentnumber, pms_departmentname, pms_stationname,college,isnull));
				}
				errorrow++;
			}
		} catch (Exception e) {
