 ...
 *
 */
@WebServlet("/Sec_ImportUnfinishedFormServlet")
public class Sec_ImportUnfinishedFormServlet extends HttpServlet
{
	...
	public Sec_ImportUnfinishedFormServlet()
	{
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		int recordcount = 0;
		List<UnfinishedForm> ufList = new ArrayList<UnfinishedForm>();

		TableListDao tableListDao = new TableListDaoImpl();
		UnfinishedFormDao ufDao = new UnfinishedFormDaoImpl();
		...
		try
		{
			...
			if (tablename.equals("附表未填表格情况"))
			{
				ufList = getAlltsByExcel(completeFilePath);
				ufDao.deleteByCollegeandDeadline(college, null);
				recordcount = ufList.size();
				for (int i = 0; i < ufList.size(); i++)
				{
					ufDao.addUnfinishedForm(ufList.get(i));
				}
			}
	 ...
	 * @return
	 */
	public static List<UnfinishedForm> getAlltsByExcel(String file)
	{
		errorrow = 1;
		List<UnfinishedForm> feList = new ArrayList<UnfinishedForm>();
		try
		{
			...
			for (int i = 2; i < rows; i++)
			{
				...
				{
					// 第一个是列数，第二个是行数
					String uf_formname = rs.getCell(j++, i).getContents();;   
					String uf_accountfor  = rs.getCell(j++, i).getContents();; 					
					...
					UnfinishedForm UnfinishedForm = new UnfinishedForm(uf_formname,
							uf_accountfor,college,1,uf_isnull);
					feList.add(UnfinishedForm);
