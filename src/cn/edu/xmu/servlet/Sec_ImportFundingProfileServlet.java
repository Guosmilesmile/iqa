...
/**
 * Servlet implementation class Sec_ImportFundingProfileServlet
 */
@WebServlet("/Sec_ImportFundingProfileServlet")
public class Sec_ImportFundingProfileServlet extends HttpServlet {
     ...
     */
    public Sec_ImportFundingProfileServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int recordcount = 0;
		List<FundingProfile> tsList = new ArrayList<FundingProfile>();
		...
		try {
			...
			if (tablename.equals("表2-10-1教学经费概况")) {
				tsList = getAlltsByExcel(completeFilePath);
				fpDao.deleteByCollegeandDeadline(college, null);
				recordcount = tsList.size();
				for (int i = 0; i < tsList.size(); i++) {
					fpDao.addRecord(tsList.get(i));
				}
			}
	 ...
	 * @return
	 */
	public static List<FundingProfile> getAlltsByExcel(String file) {
		errorrow = 1;
		List<FundingProfile> tsList = new ArrayList<FundingProfile>();
		try {
			...
			for (int i = 2; i < rows; i++) {
				for (int j = 0; j < 3; j++) {
					...
					tsList.add(new FundingProfile(fp_educationfund, fp_teachfund, fp_reformfund, college,isnull));
				}
				errorrow++;
			}
		} catch (Exception e) {
