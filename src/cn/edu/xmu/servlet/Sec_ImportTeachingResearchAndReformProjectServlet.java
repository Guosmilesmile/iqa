 ...
 *
 */
@WebServlet("/Sec_ImportTeachingResearchAndReformProjectServlet")
public class Sec_ImportTeachingResearchAndReformProjectServlet extends HttpServlet
{
	...
	public Sec_ImportTeachingResearchAndReformProjectServlet()
	{
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		int recordcount = 0;
		List<TeachingResearchAndReformProject> trarpList = new ArrayList<TeachingResearchAndReformProject>();

		TableListDao tableListDao = new TableListDaoImpl();
		TeachingResearchAndReformProjectDao trarpDao = new TeachingResearchAndReformProjectDaoImpl();
		...
		try
		{
			...
			if (tablename.equals("表7-3-1教育教学研究与改革项目（自然年）"))
			{
				trarpList = getAlltsByExcel(completeFilePath);
				trarpDao.deleteByCollegeandDeadline(college, null);
				recordcount = trarpList.size();
				for (int i = 0; i < trarpList.size(); i++)
				{
					trarpDao.addTeachingResearchAndReformProject(trarpList.get(i));
				}
			}
	 ...
	 * @return
	 */
	public static List<TeachingResearchAndReformProject> getAlltsByExcel(String file)
	{
		errorrow = 1;
		List<TeachingResearchAndReformProject> feList = new ArrayList<TeachingResearchAndReformProject>();
		try
		{
			...
			for (int i = 2; i < rows; i++)
			{
				...
				{
					// 第一个是列数，第二个是行数
					String trarp_projectname = rs.getCell(j++, i).getContents();   
					String trarp_compere  = rs.getCell(j++, i).getContents();
					String trarp_comperenumber = rs.getCell(j++, i).getContents();
				 	String trarp_level = rs.getCell(j++, i).getContents();
					String trarp_setupdate = rs.getCell(j++, i).getContents(); 
					String trarp_checkdate = rs.getCell(j++, i).getContents();  
					String funds = rs.getCell(j++, i).getContents();
					Float trarp_funds = (float) -1.0;
					if(!funds.equals("")) trarp_funds = Float.parseFloat(funds);
				    String jointeachers = rs.getCell(j++, i).getContents();
					...
					TeachingResearchAndReformProject teachingResearchAndReformProject = new TeachingResearchAndReformProject(trarp_projectname,trarp_compere,trarp_comperenumber,trarp_level,
							trarp_setupdate,trarp_checkdate,trarp_funds,trarp_jointeachers,college,1,trarp_isnull);
					
					feList.add(teachingResearchAndReformProject);
