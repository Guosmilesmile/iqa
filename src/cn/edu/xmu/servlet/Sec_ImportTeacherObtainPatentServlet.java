 ...
 *
 */
@WebServlet("/Sec_ImportTeacherObtainPatentServlet")
public class Sec_ImportTeacherObtainPatentServlet extends HttpServlet
{
	...
	public Sec_ImportTeacherObtainPatentServlet()
	{
		super();
	}
	/**
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		int recordcount = 0;
		List<TeacherObtainPatent> topList = new ArrayList<TeacherObtainPatent>();

		TableListDao tableListDao = new TableListDaoImpl();
		TeacherObtainPatentDao topDao = new TeacherObtainPatentDaoImpl();
		...
		try
		{
			...
			if (tablename.equals("表3-6-6教师获准专利数（自然年）"))
			{
				topList = getAlltsByExcel(completeFilePath);
				topDao.deleteByCollegeandDeadline(college, null);
				recordcount = topList.size();
				for (int i = 0; i < topList.size(); i++)
				{
					topDao.addTeacherObtainPatent(topList.get(i));
				}
			}
	 ...
	 * @return
	 */
	public static List<TeacherObtainPatent> getAlltsByExcel(String file)
	{
		errorrow = 1;
		List<TeacherObtainPatent> topList = new ArrayList<TeacherObtainPatent>();
		try
		{
			...
			for (int i = 2; i < rows; i++)
			{
				for (int j = 0; j < 3; j++)
				{
					...
					String invention = rs.getCell(j++, i).getContents();
					String utilitymodel = rs.getCell(j++, i).getContents();
					Integer top_total = -1;
					...
					if ("".equals(total)&&"".equals(invention)&&"".equals(utilitymodel))
						break;
					topList.add(new TeacherObtainPatent(0, top_total, top_invention, top_utilitymodel,
							college, null, 1, "", top_isnull));
				}
				errorrow++;
			}
		} catch (Exception e)
