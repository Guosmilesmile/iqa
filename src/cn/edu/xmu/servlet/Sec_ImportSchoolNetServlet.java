...
/**
 * Servlet implementation class Sec_ImportSchoolNetServlet
 */
@WebServlet("/Sec_ImportSchoolNetServlet")
public class Sec_ImportSchoolNetServlet extends HttpServlet {
     ...
     */
    public Sec_ImportSchoolNetServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int recordcount = 0;
		List<SchoolNet> snList = new ArrayList<SchoolNet>();
		...
		try {
			...
			if (tablename.equals("表2-7校园网")) {
				snList = getAllsnByExcel(completeFilePath);
				snDao.deleteByCollegeandDeadline(college, null);
				recordcount = snList.size();
				for (int i = 0; i < snList.size(); i++) {
					snDao.addRecord(snList.get(i));
				}
			}
	 ...
	 * @return
	 */
	public static List<SchoolNet> getAllsnByExcel(String file) {
		errorrow = 1;
		List<SchoolNet> snList = new ArrayList<SchoolNet>();
		try {
			...
			for (int i = 2; i < rows; i++) {
				for (int j = 1; j < 2; j++) {
					...
					snList.add(new SchoolNet(sn_backbonebandwidth, sn_exportbandwidth, sn_informationcount, college,isnull));
				}
				errorrow++;
			}
		} catch (Exception e) {
