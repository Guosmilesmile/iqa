...
/**
 * @author zhantu 
 * Servlet implementation class Sec_ImportTeacherResearchNumber
 * date 2015-07-09
 */
@WebServlet("/ImportTeacherResearchNumber")
public class Sec_ImportTeacherResearchNumber extends HttpServlet implements Servlet {
	 ...
	 */
	public Sec_ImportTeacherResearchNumber() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request,
		...
		int recordcount = 0;
		List<TeacherResearchNumber> trnList = new ArrayList<TeacherResearchNumber>();

		TableListDao tableListDao = new TableListDaoImpl();
		TeacherResearchNumberDao teachScientificDao = new TeacherResearchNumberDaoImpl();
		...
		try {
			...
			if (tablename.equals("表3-6-2教师科研项目数")) {
				trnList = getAlltrnByExcel(completeFilePath);
				teachScientificDao.deleteByCollegeandDeadline(college, null);
				recordcount = trnList.size();
				for (int i = 0; i < trnList.size(); i++) {
					teachScientificDao.addRecord(trnList.get(i));
				}
			}
	 ...
	 * @return
	 */
	public static List<TeacherResearchNumber> getAlltrnByExcel(String file) {
		errorrow = 1;
		List<TeacherResearchNumber> trnList = new ArrayList<TeacherResearchNumber>();
		try {
			...
			for (int i = 3; i < rows; i++) {
				for (int j = 1; j < 7; j++) {//j = 1 第1列是编号,第2列是标题
					// 第一个是列数，第二个是行数
					// 默认最左边编号也算一列,所以这里得j++
					j++;
					...
					trnList.add(new TeacherResearchNumber(trn_projectnum, trn_hrznprojectnum,
