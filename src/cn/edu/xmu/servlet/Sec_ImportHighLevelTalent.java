...
/**
 * 
 * @author xiaoping 表3-4-1 高层次人才(时点) date 2015-7-11
 *
 */
@WebServlet("/Sec_ImportHighLevelTalent")
public class Sec_ImportHighLevelTalent extends HttpServlet
{
	...
	public Sec_ImportHighLevelTalent()
	{
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		try
		{
			...
			if (tablename.equals("表3-4-1高层次人才（时点）"))
			{
				hltList = getAlltsByExcel(completeFilePath);
				hltDao.deleteByCollegeandDeadline(college, null);
				recordcount = hltList.size();
				for (int i = 0; i < hltList.size(); i++)
				{
					hltDao.addHighLevelTalent(hltList.get(i));
				}
			}
	 ...
	 * @return
	 */
	public static List<HighLevelTalent> getAlltsByExcel(String file)
	{
		errorrow = 1;
		List<HighLevelTalent> hltList = new ArrayList<HighLevelTalent>();
		try
		{
			...
			for (int i = 2; i < rows; i++)
			{
				...
				{
					// 第一个是列数，第二个是行数
					String hlt_name = rs.getCell(j++, i).getContents();// 默认最左边编号也算一列,所以这里得j++
					String hlt_worknumber = rs.getCell(j++, i).getContents();
					String hlt_type = rs.getCell(j++, i).getContents();
					String hlt_researchdirection = rs.getCell(j++, i).getContents();
					Date hlt_acquisitiondate = dateCellToSql(rs.getCell(j++, i));
					Date temp = Date.valueOf("1800-1-1");
					int hlt_isnull = 0;
					if ("".equals(hlt_name) || "".equals(hlt_worknumber) || "".equals(hlt_type)
							|| "".equals(hlt_researchdirection) || temp.equals(hlt_acquisitiondate))
						hlt_isnull = 1;
					if ("".equals(hlt_name) && "".equals(hlt_worknumber) && "".equals(hlt_type)
							&& "".equals(hlt_researchdirection) && temp.equals(hlt_acquisitiondate))
						break;
					hltList.add(new HighLevelTalent(0, hlt_name, hlt_worknumber, hlt_type, hlt_researchdirection,
							hlt_acquisitiondate, 1, null, college, "", hlt_isnull));
				}
				errorrow++;
			}
		} catch (Exception e)
