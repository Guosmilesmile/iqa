...
@WebServlet("/Sec_ImportMajorRegisterInfoServlet")
public class Sec_ImportMajorRegisterInfoServlet extends HttpServlet{
	 ...
	 */
	public Sec_ImportMajorRegisterInfoServlet() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request,
		...
		int recordcount = 0;
		List<MajorRegisterInfo> tsList = new ArrayList<MajorRegisterInfo>();
	 ...
	 * @return
	 */
	public static List<MajorRegisterInfo> getAlltsByExcel(String file) {
		errorrow = 1;
		List<MajorRegisterInfo> mriList = new ArrayList<MajorRegisterInfo>();
		try {
			...
			for (int i = 2; i < rows; i++) {
				for (int j = 0; j < 3; j++) {
					// 第一个是列数，第二个是行数
					String mri_majorcode = rs.getCell(j++, i).getContents();// 默认最左边编号也算一列,所以这里得j++
					String mri_majorname = rs.getCell(j++, i).getContents();
...
//					int mri_registernumber = Integer.parseInt( rs.getCell(j++, i).getContents());
					//添加修改点击保存的时候需要判断,0表示完整，1表示缺失
					int isnull = 0;
					if(mri_majorcode.equals("")||mri_majorname.equals("")||	registernumber.equals(""))
						isnull = 1;
					mriList.add(new MajorRegisterInfo(mri_majorcode, mri_majorname, mri_registernumber,
							college,isnull));
				}
				errorrow++;
			}
