...
/**
 * 表2-6-1 2014-2015学年本科实验、实习、实训场所
 * @author chunfeng
 *
 */
@WebServlet("/Sec_ImportPracticePlaceServlet")
public class Sec_ImportPracticePlaceServlet extends HttpServlet
{
	...
	public Sec_ImportPracticePlaceServlet()
	{
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		int recordcount = 0;
		List<PracticePlace> ppList = new ArrayList<PracticePlace>();

		TableListDao tableListDao = new TableListDaoImpl();
		PracticePlaceDao ppDao = new PracticePlaceDaoImpl();
		...
		try
		{
			...
			if (tablename.equals("表2-6-1本科实验、实习、实训场所"))
			{
				ppList = getAlltsByExcel(completeFilePath);
				ppDao.deleteByCollegeandDeadline(college, null);
				recordcount = ppList.size();
				for (int i = 0; i < ppList.size(); i++)
				{
					ppDao.addPracticePlace(ppList.get(i));
				}
			}
	 ...
	 * @return
	 */
	public static List<PracticePlace> getAlltsByExcel(String file)
	{
		errorrow = 1;
		List<PracticePlace> feList = new ArrayList<PracticePlace>();
		try
		{
			...
			for (int i = 2; i < rows; i++)
			{
				...
				{
					// 第一个是列数，第二个是行数
					String pp_placename = rs.getCell(j++, i).getContents();;   //名称
					String pp_collegename  = rs.getCell(j++, i).getContents();; //院系名称						
					String area = rs.getCell(j++, i).getContents();;  //面积
					Float pp_area = (float) -1.0;
					if(!"".equals(area)) pp_area = Float.parseFloat(area);
					String pp_property = rs.getCell(j++, i).getContents();; //实验室性质
				 	String pp_majors = rs.getCell(j++, i).getContents();;  //面向专业
					String hours = rs.getCell(j++, i).getContents();;  //学年度承担的实验教学人时数
					Integer pp_hours = -1;
					if(!"".equals(hours)) pp_hours = Integer.parseInt(hours);
					String times = rs.getCell(j++, i).getContents();; //学年度承担的实验教学人次数
					Integer pp_times = -1;
					if(!"".equals(times)) pp_times = Integer.parseInt(times);
					String projectnum = rs.getCell(j++, i).getContents();; //本科生实验实习实训项目数
					Integer pp_projectnum = -1;
					if(!"".equals(projectnum)) pp_projectnum = Integer.parseInt(projectnum);
					String largeststudents = rs.getCell(j++, i).getContents();;  //最大可容纳学生数
					...
					PracticePlace practicePlace = new PracticePlace(pp_placename, pp_collegename, pp_area, pp_property, pp_majors, 
							pp_hours, pp_times, pp_projectnum,pp_largeststudents,college,1,pp_isnull);
					feList.add(practicePlace);
