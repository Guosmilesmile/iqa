...
/**
 * Servlet implementation class PublishNews
 */
...
public class PublishNews extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			String date = su.getRequest().getParameter(NewsTable.N_PUBLISHTIME);
			...
			int result = 0;
			if (date.equals("")) {
				News news = new News(n_titles, n_author, n_summary, n_content,
						image_url, sclass, ssubclass, n_version);
				result = newsDao.addNewsRecord(news);
			} else {
				// string转date
				...
				try {
					...
					News news = new News(n_titles, n_author, n_summary,
							n_content, image_url, datetime, sclass, ssubclass,
							n_version);
					result = newsDao.addNewsRecord(news);
