...
/**
 * @author luo 
 * Servlet implementation class Sec_ImportServlet 
 * date 2015-07-09
 */
@WebServlet("/Sec_ImportServlet")
public class Sec_ImportServlet extends HttpServlet implements Servlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		int recordcount = 0;
		List<TeachScientific> tsList = new ArrayList<TeachScientific>();
		...
		try {
			...
			if (tablename.equals("1-4学校教学科研单位")) {
				...
				for (int i = 0; i < tsList.size(); i++) {
					teachScientificDao.addTeachScientificRecord(tsList.get(i));
				}
			}
	 ...
	 * @return
	 */
	public static List<TeachScientific> getAlltsByExcel(String file) {
		errorrow = 1;
		List<TeachScientific> tsList = new ArrayList<TeachScientific>();
		try {
			...
			for (int i = 2; i < rows; i++) {
				for (int j = 0; j < clos; j++) {
					// 第一个是列数，第二个是行数
					String ts_name = rs.getCell(j++, i).getContents();// 默认最左边编号也算一列,所以这里得j++
					String ts_number = rs.getCell(j++, i).getContents();
					String ts_head = rs.getCell(j++, i).getContents();
					
					int isnull = 0;
					if(ts_name.equals("") || ts_number.equals("") || ts_head.equals("")  )
						isnull = 1;
					
					tsList.add(new TeachScientific(ts_name, ts_number, ts_head, college,isnull));
				}
				errorrow++;
			}
		} catch (Exception e) {
