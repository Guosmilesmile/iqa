...
/**
 * @author zhantu 
 * Servlet implementation class Sec_ImportPlayGround 
 * date 2015-07-10
 */
@WebServlet("/ImportPlayGround")
public class Sec_ImportPlayGround extends HttpServlet implements Servlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		int recordcount = 0;
		List<PlayGround> tsList = new ArrayList<PlayGround>();
		...
		try {
			...
			if (tablename.equals("附表2-4-1运动场")) {
	 ...
	 * @return
	 */
	public static List<PlayGround> getAlltsByExcel(String file) {
		errorrow = 1;
		List<PlayGround> etList = new ArrayList<PlayGround>();
		try {
			...
			for (int i = 3; i < rows; i++) {
				for (int j = 0; j < 6; j++) {
					// 第一个是列数，第二个是行数
					String pg_campus = rs.getCell(j++, i).getContents();// 默认最左边编号也算一列,所以这里得j++
					String pg_groundname = rs.getCell(j++, i).getContents();
					String amount = rs.getCell(j++, i).getContents();
					int pg_amount =0 ;
					if(!amount.equals("")){
						pg_amount = Integer.parseInt(amount);
					}
					String pg_indoorarea = rs.getCell(j++, i).getContents();
					String pg_outdoorarea = rs.getCell(j++, i).getContents();
					String pg_totalarea = rs.getCell(j++, i).getContents();
					...
					etList.add(new PlayGround(pg_campus, pg_groundname, pg_amount, pg_indoorarea, pg_outdoorarea,
							pg_totalarea, college, isnull));
