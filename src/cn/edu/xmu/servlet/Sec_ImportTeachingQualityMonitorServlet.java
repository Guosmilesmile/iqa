 ...
 *
 */
@WebServlet("/Sec_ImportTeachingQualityMonitorServlet")
public class Sec_ImportTeachingQualityMonitorServlet extends HttpServlet
{
	...
	public Sec_ImportTeachingQualityMonitorServlet()
	{
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		int recordcount = 0;
		List<TeachingQualityMonitor> tqmList = new ArrayList<TeachingQualityMonitor>();

		TableListDao tableListDao = new TableListDaoImpl();
		TeachingQualityMonitorDao tqmDao = new TeachingQualityMonitorDaoImpl();
		...
		try
		{
			...
			if (tablename.equals("表7-4教学质量管理与监控（时点）"))
			{
				tqmList = getAlltsByExcel(completeFilePath);
				tqmDao.deleteByCollegeandDeadline(college, null);
				recordcount = tqmList.size();
				for (int i = 0; i < tqmList.size(); i++)
				{
					tqmDao.addTeachingQualityMonitor(tqmList.get(i));
				}
			}
	 ...
	 * @return
	 */
	public static List<TeachingQualityMonitor> getAlltsByExcel(String file)
	{
		errorrow = 1;
		List<TeachingQualityMonitor> feList = new ArrayList<TeachingQualityMonitor>();
		try
		{
			...
			for (int i = 2; i < rows; i++)
			{
				...
				{
					// 第一个是列数，第二个是行数
					String tqm_managesyscontent = rs.getCell(j++, i).getContents();;   
					String tqm_assessexe  = rs.getCell(j++, i).getContents();; 					
					String tqm_annualreport = rs.getCell(j++, i).getContents();;  
					...
					TeachingQualityMonitor teachingQualityMonitor = new TeachingQualityMonitor(tqm_managesyscontent,
							tqm_assessexe,tqm_annualreport,college,1,tqm_isnull);
					feList.add(teachingQualityMonitor);
