...
/**
 * Servlet implementation class Sec_ImportFixedAssetsServlet
 */
@WebServlet("/Sec_ImportFixedAssetsServlet")
public class Sec_ImportFixedAssetsServlet extends HttpServlet {
     ...
     */
    public Sec_ImportFixedAssetsServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int recordcount = 0;
		List<FixedAssets> faList = new ArrayList<FixedAssets>();
		...
		try {
			...
			if (tablename.equals("附表2-9固定资产")) {
				faList = getAllfaByExcel(completeFilePath);
				faDao.deleteByCollegeandDeadline(college, null);
				recordcount = faList.size();
				for (int i = 0; i < faList.size(); i++) {
					faDao.addRecord(faList.get(i));
				}
			}
	 ...
	 * @return
	 */
	public static List<FixedAssets> getAllfaByExcel(String file) {
		errorrow = 1;
		List<FixedAssets> faList = new ArrayList<FixedAssets>();
		try {
			...
			for (int i = 3; i < rows; i++) {
				for (int j = 0; j < 4; j++) {
					// 第一个是列数，第二个是行数
					String fa_importcollege = rs.getCell(j++, i).getContents();// 默认最左边编号也算一列,所以这里得j++
					String fixedassetssum = rs.getCell(j++, i).getContents();
					float fa_fixedassetssum = -999;
					if (fixedassetssum!=null && !"".equals(fixedassetssum)) {
						fa_fixedassetssum = Float.parseFloat(fixedassetssum);
					}
					
					String equipmentassetssum = rs.getCell(j++, i).getContents();
					...
					String newassets = rs.getCell(j++, i).getContents();
					...
					faList.add(new FixedAssets(fa_importcollege, fa_fixedassetssum, fa_equipmentassetssum, fa_newassets,college,isnull));
				}
				errorrow++;
			}
		} catch (Exception e) {
