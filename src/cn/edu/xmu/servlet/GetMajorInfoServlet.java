 ...
 * 4-2-2-1 专业基本情况
 */
@WebServlet("/GetMajorInfoServlet")
public class GetMajorInfoServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {				
		...
		MajorInfoDao midao = new MajorInfoDaoImpl();
		GridDataModel<MajorInfo> model = new GridDataModel<MajorInfo>();
	    ...
	    if(isnull != null && !"".equals(isnull)){
	    	queryParams.put(MajorInfoTable.ISNULL, isnull);
	    }
	    
	    int total = midao.getCount(queryParams);
	    if(null == pageStr){
	    	pageStr = "1";
	    }if(null == rowsStr){
	    	rowsStr=total+"";
	    ...
	    model.setRows(midao.getMajorInfo(start, end, sortStr, orderStr, queryParams));	    
